...
//		int[] otherIndexList = sg.getOtherIndexArray(index);
	...
	private static boolean compatible(edu.utah.ece.async.dataModels.verification.platu.stategraph.State currentState, Constraint constr, int[] thisIndexList, int[] otherIndexList){
     ...
     */
	@SuppressWarnings("unused")
	private static int createNewState(StateGraph sg, edu.utah.ece.async.dataModels.verification.platu.stategraph.State compatibleState, Constraint c){
		int newTransitions = 0;

		// Create new state and insert into state graph
		edu.utah.ece.async.dataModels.verification.platu.stategraph.State newState = new edu.utah.ece.async.dataModels.verification.platu.stategraph.State(compatibleState);
		...
		edu.utah.ece.async.dataModels.verification.platu.stategraph.State nextState = sg.addState(newState);
