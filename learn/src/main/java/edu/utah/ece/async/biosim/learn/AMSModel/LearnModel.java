 ...
 *  
 *******************************************************************************/
package edu.utah.ece.async.biosim.learn.AMSModel;
...
import edu.utah.ece.async.biosim.dataModels.util.dataparser.*;
...
public class LearnModel extends Observable { // added ItemListener SB
  ...
  public LPN learnModel(String directory, int moduleNumber, HashMap<String, 
    ...
    try {
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "LPN file couldn't be created/written.");
      this.notifyObservers(message);
    }
    catch (NullPointerException e4) {
      out.close();
      e4.printStackTrace();
      message.setErrorDialog("ERROR!", "LPN file couldn't be created/written. Null exception");
      this.notifyObservers(message);
    }
    catch (ArrayIndexOutOfBoundsException e1) {	// comes from initMark = -1 of updateGraph()
      e1.printStackTrace();
      message.setErrorDialog("ERROR!", "Unable to calculate rates.\nWindow size or pathLengthBin must be reduced.\nLearning unsuccessful.");
      this.notifyObservers(message);
      try {
      ...
      message.setErrorDialog("ERROR!", "LPN File not found for merging.");
      this.notifyObservers(message);
    }
    //return (g); 
  ...
  public Properties addInitPlace(HashMap<String,ArrayList<Double>> scaledThresholds){
    ...
    try{
      try{
        ...
        e.printStackTrace();
        message.setErrorDialog("ERROR!", "Null exception in addInitPlace.");
        this.notifyObservers(message);
        out.close();
      }
    }catch (IOException e) {
      ...
      message.setErrorDialog("ERROR!", "Log file couldn't be opened in addInitPlace.");
      this.notifyObservers(message);
    }
  ...
  private void genBinsRates(HashMap<String, ArrayList<Double>> localThresholds) { 
    ...
    try{
      ...
      for (int i = 0; i < reqdVarsL.size(); i++) {
        ...
        for (int j = 1; j < varNames.size(); j++) {
          ...
          if (currentVar.equalsIgnoreCase(varNames.get(j))) {
            ...
            if (binError)
            {
              message.setErrorDialog("ERROR!", "Bin couldn't be retrieved for a point. Please check thresholds.");
              this.notifyObservers(message);
            }
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Log file couldn't be opened for writing genBinsRates messages.");
      this.notifyObservers(message);
    }
  ...
  public void updateRateInfo(int[][] bins, Double[][] rates, int traceNum, Boolean transientPlaceReqd, Properties cvgProp) {
    ...
    try{
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Log file couldn't be opened for writing UpdateRateInfo messages.");
      this.notifyObservers(message);
      /*	}catch (NullPointerException e4) {
			e4.printStackTrace();
      ...
      e4.printStackTrace();
      message.setErrorDialog("ERROR!",  "The number of thresholds generated do not match with the dmvc values.    Use 'Auto' option.");
      this.notifyObservers(message);
    }
  }
  ...
  public HashMap<String, ArrayList<Double>> detectDMV(ArrayList<ArrayList<Double>> data, Boolean callFromAutogen) {
    ...
    try{
      ...
      e.printStackTrace();
      //System.out.println("LPN file couldn't be created/written ");
      message.setErrorDialog("ERROR!","Log file couldn't be opened for writing rates and bins.");
      this.notifyObservers(message);
    }
    //dmvDetectDone = true;
    return(dmvDivisions);
  ...
  public HashMap<String,ArrayList<Double>> scaleValue(Double scaleFactor, HashMap<String,ArrayList<Double>> localThresholds) {
    ...
    try{
      ...
      e.printStackTrace();
      //System.out.println("LPN file couldn't be created/written ");
      message.setErrorDialog("ERROR!", "Not all regions have values for all dmv variables");
      this.notifyObservers(message);
    }
  ...
  public void scaleDelay(Double scaleFactor) {
    try{
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Delay scaling error due to null. Check");
      this.notifyObservers(message);
    }
    catch(java.lang.ArrayIndexOutOfBoundsException e){
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Delay scaling error due to Array Index.");
      this.notifyObservers(message);
    }
  }
  ...
  public void addPseudo(HashMap<String,ArrayList<Double>> scaledThresholds){
    try{
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Log file couldn't be written.");
      this.notifyObservers(message);
    }
  }
  ...
  public void writeVHDLAMSFile(String vhdFile){
    try{
      ...
      message.setErrorDialog("ERROR!", "VHDL-AMS model couldn't be created/written.");
      this.notifyObservers(message);
    }
    catch(Exception e){
      message.setErrorDialog("ERROR!", "Error in VHDL-AMS model generation.");
      this.notifyObservers(message);
    }
  }
  public void writeVerilogAMSFile(String vamsFileName){
    try{
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Verilog-AMS model couldn't be created/written.");
      this.notifyObservers(message);
    }
    catch(Exception e){
      message.setErrorDialog("ERROR!", "Error in Verilog-AMS model generation.");
      this.notifyObservers(message);
    }
  }
  //T[] aux = (T[])a.clone();
  ...
  public LPN mergeLhpns(LPN l1,LPN l2){//(LhpnFile l1, LhpnFile l2){
    ...
    try{
      ...
      e.printStackTrace();
      message.setErrorDialog("ERROR!", "Problem while merging lpns");
      this.notifyObservers(message);
    }
    //return l1;
    return l3;
