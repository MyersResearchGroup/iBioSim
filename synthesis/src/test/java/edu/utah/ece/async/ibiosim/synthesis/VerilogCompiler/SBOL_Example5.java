...
/**
 * Test NOR gate design that was compiled from a verilog expression to an SBOL data model
 * @author Tramy Nguyen
 *
 */
public class SBOL_Example5 extends AbstractVerilogParserTest{
	...
	public static void setupTest() {
		...
		String[] cmd = {"-v", reader.getFile("contAssign6.v"), "-sbol", "-od", CompilerTestSuite.outputDirectory};
	...
	public void Test_cdSize() {
		Assert.assertEquals(4, sbolDoc.getComponentDefinitions().size());
	}
	...
	public void Test_CD_gate() {
		List<String> expected_id = Arrays.asList("CD3_norGate");
		for(String id : expected_id) {
			ComponentDefinition cd = sbolDoc.getComponentDefinition(id, "1.0");
			Assert.assertNotNull(cd);
			Assert.assertEquals(1, cd.getTypes().size());
	...
	public void Test_FC_output() {
		FunctionalComponent fc = sbolDesign.getFunctionalComponent("FC2_y");
		Assert.assertNotNull(fc);
		Assert.assertEquals(AccessType.PUBLIC, fc.getAccess());
	...
	public void Test_FC_gate() {
		...
		FunctionalComponent fc = sbolDesign.getFunctionalComponent("FC3_norGate");
		Assert.assertNotNull(fc);
		Assert.assertEquals(AccessType.PUBLIC, fc.getAccess());
	...
	public void Test_Interaction1() {
		...
		Assert.assertEquals(sbolDesign.getFunctionalComponent("FC3_norGate"), p2.getParticipant());
	...
	public void Test_Interaction3() {
		...
		Assert.assertNotNull(p1);
		Assert.assertEquals(sbolDesign.getFunctionalComponent("FC3_norGate"), p1.getParticipant());
		Assert.assertEquals(1, p1.getRoles().size());
