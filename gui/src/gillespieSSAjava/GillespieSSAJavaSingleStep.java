...
//	 }
	 ...
	 public double evaluatePropensityFunction(ASTNode currentASTNode, HashMap<String, Double>ListOfLocalParameters) {
		...
		if(isLeafNode(currentASTNode)){
		   ret = evalChild(currentASTNode, ListOfLocalParameters);	  
		}
	  ...
	  public boolean isLeafNode(ASTNode node ){
		  boolean ret = false;
		  ret = node.isConstant() || node.isInteger() || node.isName() || node.isNumber() || node.isRational() || node.isReal();
		  return ret;
	  }
	  
	  public double evalChild(ASTNode currentASTNode, HashMap<String, Double> currentListOfLocalParams){
		  double node_val=0;
