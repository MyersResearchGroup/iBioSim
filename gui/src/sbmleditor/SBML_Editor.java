...
public class SBML_Editor extends JPanel implements ActionListener, MouseListener {
	...
	private JComboBox substanceUnits, timeUnits, volumeUnits, areaUnits, lengthUnits, extentUnits,
			conversionFactor;
	...
	private JComboBox specUnits, initLabel, stoiciLabel, specConv;

	private JComboBox comp, reactionComp; // compartment combo box
	...
	private void createSbmlFrame(String file) {
		...
		if (!file.equals("")) {
			...
			// createFunction(model, "priority", "Priority expression",
			// "lambda(d,p,d)");
		}
		else {
	...
	private JPanel createRuleFrame(Model model) {
		...
		for (int i = 0; i < model.getNumRules(); i++) {
			...
			if (rule.isAlgebraic()) {
				...
				rul[i] = "d( " + rule.getVariable() + " )/dt = "
						+ myFormulaToString(rule.getMath());
			}
		}
		String[] oldRul = rul;
		try {
		...
		JPanel eventPanel = createPanel(model, "Events", events, ev, addEvent, removeEvent,
				editEvent);
	...
	private void removeList() {
		...
		if (index != -1) {
			...
			uList = (String[]) Utility.remove(unitList, uList);
	...
	private void removeUnit() {
		...
		if (index != -1) {
			if (!unitsInUse(((String) unitDefs.getSelectedValue()).split(" ")[0])) {
				...
				units = (String[]) Utility.remove(unitDefs, units);
	...
	private void removeCompType() {
		...
		if (index != -1) {
			...
			if (remove) {
				...
				cpTyp = (String[]) Utility.remove(compTypes, cpTyp);
	...
	private void removeInit() {
		...
		if (index != -1) {
			...
			inits = (String[]) Utility.remove(initAssigns, inits);
	...
	private void removeSpecType() {
		...
		if (index != -1) {
			...
			if (remove) {
				...
				spTyp = (String[]) Utility.remove(specTypes, spTyp);
	...
	private void removeRule() {
		...
		if (index != -1) {
			...
			rul = (String[]) Utility.remove(rules, rul);
	...
	private void removeEvent() {
		...
		if (index != -1) {
			...
			ev = (String[]) Utility.remove(events, ev);
	...
	private void removeAssignment() {
		...
		if (index != -1) {
			...
			assign = (String[]) Utility.remove(eventAssign, assign);
	...
	private void removeConstraint() {
		...
		if (index != -1) {
			...
			cons = (String[]) Utility.remove(constraints, cons);
		...
		if (index != -1) {
			if (document.getModel().getNumCompartments() != 1) {
				...
				if (!remove) {
					...
					comps = (String[]) Utility.remove(compartments, comps);
	...
	private void removeSpecies() {
		...
		if (index != -1) {
			if (!variableInUse(((String) species.getSelectedValue()).split(" ")[0], false)) {
				...
				specs = (String[]) Utility.remove(species, specs);
				species.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	...
	private void copyReaction() {
		...
		if (!usedIDs.contains(reacID.trim()) && !reacID.trim().equals("")) {
			...
			adding = Utility.add(reacts, reactions, add, false, null, null, null, null, null, null,
					Gui.frame);
	...
	private void removeParameter() {
		...
		if (index != -1) {
			if (!variableInUse(((String) parameters.getSelectedValue()).split(" ")[0], false)) {
				...
				params = (String[]) Utility.remove(parameters, params);
				parameters.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		...
		if (index != -1) {
			...
			for (int i = 0; i < changedReactants.size(); i++) {
				if (changedReactants.get(i).getSpecies().equals(v)
						&& !variableInUse(changedReactants.get(i).getId(), false)) {
	...
	private void removeFunction() {
		...
		if (index != -1) {
			if (!variableInUse(((String) functions.getSelectedValue()).split(" ")[0], false)) {
				...
				funcs = (String[]) Utility.remove(functions, funcs);
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (!error) {
				if (eqn.getText().trim().equals("")) {
					...
					ArrayList<String> invalidVars = getInvalidVariables(eqn.getText().trim(),
							false, args.getText().trim(), true);
					...
					if (!error && document.getLevel() < 3) {
						if (addUnit.equals("substance")) {
							if (!(extractUnitKind(uList[0]).equals("dimensionless")
									...
									uList[0]).equals("kilogram") && Integer
									.valueOf(extractUnitExp(uList[0])) == 1))) {
								JOptionPane
										.showMessageDialog(
												Gui.frame,
	...
	private void initEditor(String option) {
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (initMath.getText().trim().equals("")) {
				...
				ArrayList<String> invalidVars = getInvalidVariables(initMath.getText().trim(),
						false, "", false);
				if (invalidVars.size() > 0) {
					String invalid = "";
					for (int i = 0; i < invalidVars.size(); i++) {
			...
			if (!error) {
				if (option.equals("OK")) {
					...
					if (error) {
						...
						for (int i = 0; i < document.getModel().getNumInitialAssignments(); i++) {
							if (myFormulaToString(((InitialAssignment) ia.get(i)).getMath())
									.equals(myFormulaToString(r.getMath()))
	...
	private void ruleEditor(String option) {
		...
		if (option.equals("OK")) {
			...
			if ((selected.split(" ")[0]).equals("0")) {
				...
				for (int i = 0; i < document.getModel().getNumRules(); i++) {
					if ((((Rule) r.get(i)).isAlgebraic())
							&& (myFormulaToString(((Rule) r.get(i)).getMath()).equals(ruleMath
									.getText()))) {
						Rindex = i;
					}
				}
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (ruleMath.getText().trim().equals("")) {
				...
				ArrayList<String> invalidVars = getInvalidVariables(ruleMath.getText().trim(),
						false, "", false);
				if (invalidVars.size() > 0) {
					String invalid = "";
					for (int i = 0; i < invalidVars.size(); i++) {
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (!error) {
				if (consMath.getText().trim().equals("")
					...
					ArrayList<String> invalidVars = getInvalidVariables(consMath.getText().trim(),
							false, "", false);
					if (invalidVars.size() > 0) {
						String invalid = "";
						for (int i = 0; i < invalidVars.size(); i++) {
					...
					if (option.equals("OK")) {
						...
						if (paramsOnly) {
							...
							for (int i = 0; i < parameterChanges.size(); i++) {
								if (parameterChanges.get(i).split(" ")[0].equals(compID.getText()
										.trim())) {
									remove = i;
								}
							}
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (!error) {
				if (kineticLaw.getText().trim().equals("")) {
					...
					if (!error) {
						error = checkNumFunctionArguments(myParseFormula(kineticLaw.getText()
								.trim()));
					}
				}
			}
	...
	private void modelUnitsEditor(String option) {
		...
		for (int i = 0; i < document.getModel().getNumUnitDefinitions(); i++) {
			...
			if ((unit.getNumUnits() == 1) && (unit.getUnit(0).isSecond())
					&& (unit.getUnit(0).getExponentAsDouble() == 1)) {
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (conversionFactor.getSelectedItem().equals("( none )")) {
				...
				document.getModel()
						.setConversionFactor((String) conversionFactor.getSelectedItem());
			}
			dirty = true;
			if (error) {
				...
				if (!error) {
					...
					if (option.equals("OK")) {
						...
						if (paramsOnly) {
							...
							kineticLaw.setText(updateFormulaVar(kineticLaw.getText().trim(), v,
									reacParamID.getText().trim()));
						}
					}
					else {
	...
	public void productsEditor(String option) {
		...
		for (int i = 0; i < speciesList.length; i++) {
			...
			if (species.getBoundaryCondition()
					|| (!species.getConstant() && keepVar("", speciesList[i], false, true, false,
							false))) {
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (productId.getText().trim().equals("")) {
				error = variableInUse(selectedID, false);
			}
			else {
				error = checkID(productId.getText().trim(), selectedID, false);
			}
			if (!error) {
			...
			if (!error) {
				...
				products.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
				product = Utility.getList(product, products);
				products.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
				if (index >= 0) {
					products.setSelectedIndex(index);
				}
			...
			if (!error) {
				if (stoiciLabel.getSelectedItem().equals("Stoichiometry Math")) {
					if (productStoiciometry.getText().trim().equals("")) {
						...
						ArrayList<String> invalidVars = getInvalidVariables(productStoiciometry
						...
						if (!error) {
							error = checkNumFunctionArguments(myParseFormula(productStoiciometry
									.getText().trim()));
						}
						if (!error) {
			...
			if (!error && option.equals("OK") && productConstant.getSelectedItem().equals("true")) {
				...
				error = checkConstant("Product stoiciometry", id);
			}
			if (!error) {
				if (option.equals("OK")) {
					String v = ((String) products.getSelectedValue()).split(" ")[0];
					...
					if (stoiciLabel.getSelectedItem().equals("Stoichiometry")) {
						...
						produ.getStoichiometryMath().setMath(
								myParseFormula(productStoiciometry.getText().trim()));
					...
					products.setListData(product);
					products.setSelectedIndex(index);
					updateVarId(false, selectedID, productId.getText().trim());
					kineticLaw.setText(updateFormulaVar(kineticLaw.getText().trim(), selectedID,
					...
					add.setSelectedIndex(0);
					products.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
					adding = Utility.add(product, products, add, false, null, null, null, null,
							null, null, Gui.frame);
	...
	public void reactantsEditor(String option) {
		...
		/*
		 * int j = 0; while (usedIDs.contains("reactant"+j)) { j++; }
		 * reactantId.setText("reactant"+j);
		 */
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (reactantId.getText().trim().equals("")) {
				...
				error = checkID(reactantId.getText().trim(), selectedID, false);
			}
			if (!error) {
				if (stoiciLabel.getSelectedItem().equals("Stoichiometry")) {
					try {
			...
			if (!error) {
				if (stoiciLabel.getSelectedItem().equals("Stoichiometry Math")) {
					if (reactantStoiciometry.getText().trim().equals("")) {
						...
						if (!error) {
							error = checkNumFunctionArguments(myParseFormula(reactantStoiciometry
									.getText().trim()));
						}
						if (!error) {
			...
			if (!error && option.equals("OK") && reactantConstant.getSelectedItem().equals("true")) {
				String id = selectedID;
				error = checkConstant("Reactant stoiciometry", id);
			}
			if (!error) {
				if (option.equals("OK")) {
					String v = ((String) reactants.getSelectedValue()).split(" ")[0];
					...
					if (stoiciLabel.getSelectedItem().equals("Stoichiometry")) {
						...
						reactan.getStoichiometryMath().setMath(
								myParseFormula(reactantStoiciometry.getText().trim()));
					...
					reactants.setListData(reacta);
					reactants.setSelectedIndex(index);
					updateVarId(false, selectedID, reactantId.getText().trim());
					kineticLaw.setText(updateFormulaVar(kineticLaw.getText().trim(), selectedID,
		...
		try {
			...
			for (String s : elementChanges) {
				...
				for (long i = d.getModel().getNumRules() - 1; i >= 0; i--) {
					if (s.contains("=")) {
						String formula = myFormulaToString(((Rule) d.getModel().getListOfRules()
								.get(i)).getMath());
						String sFormula = s.substring(s.indexOf('=') + 1).trim();
						sFormula = myFormulaToString(myParseFormula(sFormula));
						sFormula = s.substring(0, s.indexOf('=') + 1) + " " + sFormula;
