...
public class SBML_Editor extends JPanel implements ActionListener, MouseListener {
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (!error) {
				if (option.equals("OK")) {
					...
					// JOptionPane.showMessageDialog(biosim.frame(), "Kinetic law contains
					// literals numbers or parameters with undeclared units.\n" +
					// "Therefore, it is not possible to completely verify the consistency
					// of the units.",
					// "Contains Undeclared Units", JOptionPane.WARNING_MESSAGE);
					// }
					/*
					 * UnitDefinition unitDef =
					 * react.getKineticLaw().getDerivedUnitDefinition(); for (int i = 0; i <
					 * unitDef.getNumUnits(); i++) { Unit unit = unitDef.getUnit(i);
	...
	public boolean checkRuleUnits(Rule rule) {
		...
		// JOptionPane.showMessageDialog(biosim.frame(), "Rule contains literals
		// numbers or parameters with undeclared units.\n" +
		...
		for (int i = 0; i < unitDef.getNumUnits(); i++) {
			...
			System.out.println(unit.getKind() + " Exp = " + unit.getExponent() + " Mult = "
					+ unit.getMultiplier() + " Scale = " + unit.getScale());
		}
		// }
		return false;
	}
