...
public class SBML_Editor extends JPanel implements ActionListener, MouseListener {
	...
	private ArrayList<Parameter> oldParameters; // ArrayList of parameters
	...
	private JPanel createRuleFrame(Model model) {
		...
		if (!cycle && checkCycles(inits,rul)) {
		  JOptionPane.showMessageDialog(biosim.frame(), "Cycle detected within initial assignments and assignment rules.",
						"Cycle Detected", JOptionPane.ERROR_MESSAGE);
		}
	...
	private void copyReaction() {
	  ...
	  if (!usedIDs.contains(reacID.trim()) && !reacID.trim().equals("")) {
	    ...
	    Reaction r = document.getModel().getReaction(((String) reactions.getSelectedValue()).split(" ")[0]);
	    for (int i = 0; i < r.getKineticLaw().getNumParameters(); i++) {
	      react.getKineticLaw().addParameter((Parameter) r.getKineticLaw().getListOfParameters().get(i));
	    }
	...
	private void useMassAction() {
	  ...
	  for (SpeciesReference s : changedReactants) {
	    if (s.isSetStoichiometryMath()) {
	      kinetic += " * pow(" + s.getSpecies() + ", " + myFormulaToString(s.getStoichiometryMath().getMath()) + ")";
	    } else {
	...
	private void reacRemoveParam() {
	  if (reacParameters.getSelectedIndex() != -1) {
		...
		if (reaction.getProduct(j).isSetStoichiometryMath()) {
		  vars = myFormulaToString(reaction.getProduct(j).getStoichiometryMath().getMath()).split(" |\\(|\\)|\\,");
		...
		if (reaction.getReactant(j).isSetStoichiometryMath()) {
		  vars = myFormulaToString(reaction.getReactant(j).getStoichiometryMath().getMath()).split(" |\\(|\\)|\\,");
		  for (int k = 0; k < vars.length; k++) {
		    if (vars[k].equals(v)) {
		      JOptionPane.showMessageDialog(biosim.frame(),
	...
	private boolean variableInUse(String species,boolean zeroDim) {
	  ...
	  for (int i = 0; i < model.getNumReactions(); i++) {
		...
		if (species.equals(specRef)) {
		  ...
		  String [] vars = myFormulaToString(reaction.getProduct(j).getStoichiometryMath().getMath()).split(" |\\(|\\)|\\,");
		  for (int k = 0; k < vars.length; k++) {
		...
		if (species.equals(specRef)) {
		  ...
		  String [] vars = myFormulaToString(reaction.getReactant(j).getStoichiometryMath().getMath()).split(" |\\(|\\)|\\,");
		  for (int k = 0; k < vars.length; k++) {
		    if (vars[k].equals(species)) {
		      stoicMathUsing.add(reaction.getId() + "/" + specRef);
		      inUse = true;
	...
	private boolean checkID(String ID,String selectedID,boolean isReacParam) {
	  if (ID.equals("")) {
	    JOptionPane.showMessageDialog(biosim.frame(),"An ID is required.","Enter an ID", JOptionPane.ERROR_MESSAGE);
	    return true;
	  }
	  if (!(IDpat.matcher(ID).matches())) {
	    JOptionPane.showMessageDialog(biosim.frame(),"An ID can only contain letters, numbers, and underscores.",
					  "Invalid ID", JOptionPane.ERROR_MESSAGE);
	    return true;
	  }
	  if (ID.equals("t") || ID.equals("t") || ID.equals("true") || ID.equals("false") || ID.equals("notanumber") ||
	      ID.equals("pi") || ID.equals("infinity") || ID.equals("exponentiale") || ID.equals("abs") 
		|| ID.equals("arccos") || ID.equals("arccosh") || ID.equals("arcsin") || ID.equals("arcsinh") 
		|| ID.equals("arctan") || ID.equals("arctanh") || ID.equals("arccot") || ID.equals("arccoth")
		|| ID.equals("arccsc") || ID.equals("arccsch") || ID.equals("arcsec") || ID.equals("arcsech")
		|| ID.equals("acos") || ID.equals("acosh") || ID.equals("asin") || ID.equals("asinh") 
		|| ID.equals("atan") || ID.equals("atanh") || ID.equals("acot") || ID.equals("acoth")
		|| ID.equals("acsc") || ID.equals("acsch") || ID.equals("asec") || ID.equals("asech")
		|| ID.equals("cos") || ID.equals("cosh") || ID.equals("cot") || ID.equals("coth")
		|| ID.equals("csc") || ID.equals("csch") || ID.equals("ceil") || ID.equals("factorial")
		|| ID.equals("exp") || ID.equals("floor") || ID.equals("ln") || ID.equals("log") 
		|| ID.equals("sqr") || ID.equals("log10") || ID.equals("pow") || ID.equals("sqrt")
		|| ID.equals("root") || ID.equals("piecewise") || ID.equals("sec") || ID.equals("sech")
	    ...
	    JOptionPane.showMessageDialog(biosim.frame(),"ID cannot be a reserved word.", "Illegal ID",JOptionPane.ERROR_MESSAGE);
	    return true;
	  }
	  if (usedIDs.contains(ID) && !ID.equals(selectedID)) {
	    if (isReacParam) {
	      JOptionPane.showMessageDialog(biosim.frame(),"ID shadows a global ID.", "Not a Unique ID",JOptionPane.WARNING_MESSAGE);
	    } else {
	      JOptionPane.showMessageDialog(biosim.frame(),"ID is not unique.", "Enter a Unique ID",JOptionPane.ERROR_MESSAGE);
	      return true;
	    }
	  }
	  return false;
	...
	private void unitEditor(String option) {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
	    ...
	    if (unitID.getText().trim().equals("")) {
	      JOptionPane
		.showMessageDialog(biosim.frame(), "A unit definition ID is required.",
		...
		if (!error) {
		  if (addUnit.equals("substance")) {
		    ...
		    if (!(extractUnitKind(uList[0]).equals("dimensionless")||
		      ...
		      JOptionPane.showMessageDialog(biosim.frame(),
						    "Redefinition of time must be dimensionless or in terms of seconds.", "Incorrect Redefinition",
						    JOptionPane.ERROR_MESSAGE);
		      error = true;
		    }
		...
		if (option.equals("OK")) {
		  ...
		  for (int i = 0; i < uList.length; i++) {
		    Unit unit = new Unit(extractUnitKind(uList[i]), Integer.valueOf(extractUnitExp(uList[i])).intValue(), 
					 Integer.valueOf(extractUnitScale(uList[i])).intValue(), 
					 Double.valueOf(extractUnitMult(uList[i])).doubleValue());
		    u.addUnit(unit);
		  }
		  units[index] = addUnit;
		  sort(units);
		  ...
		  usedIDs.add(addUnit);
		  for (int i = 0; i < uList.length; i++) {
		    Unit unit = new Unit(extractUnitKind(uList[i]), 
					 Integer.valueOf(extractUnitExp(uList[i])).intValue(), 
					 Integer.valueOf(extractUnitScale(uList[i])).intValue(), 
					 Double.valueOf(extractUnitMult(uList[i])).doubleValue());
		    u.addUnit(unit);
		  }
	...
	private void initEditor(String option) {
	  ...
	  if (option.equals("OK")) {
	    ...
	    ListOf r = document.getModel().getListOfInitialAssignments();
	    for (int i = 0; i < document.getModel().getNumInitialAssignments(); i++) {
	      if (myFormulaToString(((InitialAssignment) r.get(i)).getMath()).equals(
										     initMath.getText())
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
	    ...
	    if (initMath.getText().trim().equals("")) {
	      ...
	      ArrayList<String> invalidVars = getInvalidVariables(initMath.getText().trim(),false,"",false);
	      if (invalidVars.size() > 0) {
		String invalid = "";
		for (int i = 0; i < invalidVars.size(); i++) {
		  if (i == invalidVars.size() - 1) {
		...
		if (myParseFormula(initMath.getText().trim()).isBoolean()) {
		  JOptionPane.showMessageDialog(biosim.frame(), "Initial assignment must evaluate to a number.",
		...
		if (error) {
		  ...
		  for (int i = 0; i < document.getModel().getNumInitialAssignments(); i++) {
		    if (myFormulaToString(((InitialAssignment) ia.get(i)).getMath()).equals(myFormulaToString(r.getMath()))
	...
	private void ruleEditor(String option) {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
	    ...
	    if (ruleMath.getText().trim().equals("")) {
	      ...
	      ArrayList<String> invalidVars = getInvalidVariables(ruleMath.getText().trim(),false,"",false);
	      if (invalidVars.size() > 0) {
		String invalid = "";
		for (int i = 0; i < invalidVars.size(); i++) {
		  if (i == invalidVars.size() - 1) {
		...
		if (ruleType.getSelectedItem().equals("Algebraic")) {
		  ...
		  addStr = "d( " + addVar + " )/dt = " + myFormulaToString(myParseFormula(ruleMath.getText().trim()));
		}
		else {
	...
	private void eventEditor(String option) {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
	    ...
	    if (eventTrigger.getText().trim().equals("")) {
	      ...
	      ArrayList<String> invalidVars = getInvalidVariables(eventTrigger.getText().trim(),false,"",false);
	      if (invalidVars.size() > 0) {
		String invalid = "";
		for (int i = 0; i < invalidVars.size(); i++) {
		  if (i == invalidVars.size() - 1) {
		...
		if (!error) {
		  if (myParseFormula(eventDelay.getText().trim()).isBoolean()) {
		    JOptionPane.showMessageDialog(biosim.frame(), "Event delay must evaluate to a number.",
						  "Number Expected", JOptionPane.ERROR_MESSAGE);
		    error = true;
		  }	    
		}
	...
	private void eventAssignEditor(String option) {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		if (myParseFormula(eqn.getText().trim()).isBoolean()) {
		  JOptionPane.showMessageDialog(biosim.frame(), "Event assignment must evaluate to a number.",
						"Number Expected", JOptionPane.ERROR_MESSAGE);
		  error = true;
		}	    
	      }
	...
	private void constraintEditor(String option) {
	  ...
	  if (option.equals("OK")) {
		...
		if (((Constraint) c.get(i)).isSetMessage()) {
		  XMLNode xmlNode = ((Constraint) c.get(i)).getMessage();
		  String message = xmlNode.convertXMLNodeToString(xmlNode);
		  message = message.substring(message.indexOf("xhtml\">")+7,
					      message.indexOf("</p>"));
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		JOptionPane.showMessageDialog(biosim.frame(), "Constraint formula must be of type Boolean.",
					      "Enter Valid Formula", JOptionPane.ERROR_MESSAGE);
		error = true;
	      }
	      else {
		ArrayList<String> invalidVars = getInvalidVariables(consMath.getText().trim(),false,"",false);
		if (invalidVars.size() > 0) {
		  String invalid = "";
		  for (int i = 0; i < invalidVars.size(); i++) {
		    if (i == invalidVars.size() - 1) {
		...
		if (option.equals("OK")) {
		  ...
		  if (!consMessage.getText().trim().equals("")) {
		    ...
		    xmlNode = xmlNode.convertStringToXMLNode("<message><p xmlns=\"http://www.w3.org/1999/xhtml\">" + consMessage.getText().trim() + "</p></message>");
	...
	private void compartEditor(String option) {
	  ...
	  if (option.equals("OK")) {
		...
		compConstant.setSelectedItem("false");
	      ...
	      if (paramsOnly
		  && (((String) compartments.getSelectedValue()).contains("Custom") || 
		      ((String) compartments.getSelectedValue()).contains("Sweep"))) {
	    ...
	    type.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
					  if (!((String) type.getSelectedItem()).equals("Original")) {
					    ...
					    if (d.getModel().getCompartment(((String) compartments.getSelectedValue()).split(" ")[0])
						.isSetSize()) {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		if (!splits[splits.length-2].equals("Custom") && !splits[splits.length-2].equals("Sweep")) {
		...
		if (((String) type.getSelectedItem()).equals("Sweep")) {
		  try {
		    ...
		    addComp += "Sweep (" + startVal + "," + stopVal + "," + stepVal + "," + level.getSelectedItem() + ")";
		  } catch (Exception e1) {
		    error = true;
		    JOptionPane.showMessageDialog(biosim.frame(),
		...
		if (option.equals("OK")) {
		  ...
		  String addStr;
		  if (!selCompType.equals("( none )")) {
		    addStr = compID.getText().trim() + " " + selCompType + " " + compSize.getText().trim();
		  }
		  else {
	...
	private boolean checkOutsideCycle(String compID,String outside,int depth) {
	  depth++;
	  if (depth > document.getModel().getNumCompartments()) return true;
	...
	private void speciesEditor(String option) {
	  ...
	  if (option.equals("OK")) {
		...
		initLabel.setSelectedItem("Initial Concentration");
	      ...
	      if (paramsOnly && (((String) species.getSelectedValue()).contains("Custom") || 
				 ((String) species.getSelectedValue()).contains("Sweep"))) {
		...
		for (int i=0; i<document.getModel().getNumSpecies(); i++) {
		  if (!listOfSpecies.get(i).getId().equals(selected)) {
			...
			((Species)listOfSpecies.get(i)).getSpeciesType().equals(selSpecType)) {
		      JOptionPane.showMessageDialog(biosim.frame(),
						    "Compartment already contains another species of this type.", "Species Type Not Unique",
						    JOptionPane.ERROR_MESSAGE);
		      error = true;
		    }
		  }
	...
	private void reactionsEditor(String option) {
	  ...
	  String selectedID = "";
	  if (option.equals("OK")) {
	    Reaction reac = document.getModel().getReaction(((String) reactions.getSelectedValue()).split(" ")[0]);
	  ...
	  if (option.equals("OK")) {
		...
		reacta[i] = reactant.getSpecies() + " " + myFormulaToString(reactant.getStoichiometryMath().getMath());
	      } else {
	  ...
	  if (option.equals("OK")) {
		...
		this.product[i] = product.getSpecies() + " " + myFormulaToString(product.getStoichiometryMath().getMath());
	      } else {
	  ...
	  if (option.equals("OK")) {
	    kineticLaw.setText(document.getModel().getReaction(((String) reactions.getSelectedValue()).split(" ")[0]).getKineticLaw().getFormula());
	  }
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		ArrayList<String> invalidKineticVars = getInvalidVariables(kineticLaw.getText().trim(),true,"",false);
		...
		String oldLaw = react.getKineticLaw().getFormula();
	 ...
	 */
	private ArrayList<String> getInvalidVariables(String formula,boolean isReaction,String arguments,boolean isFunction) {
	  ...
	  for (int i = 0; i < splitLaw.length; i++) {
		...
		|| splitLaw[i].equals("ceil") || splitLaw[i].equals("factorial")
		|| splitLaw[i].equals("exp") || splitLaw[i].equals("floor")
		|| splitLaw[i].equals("ln") || splitLaw[i].equals("log") 
		|| splitLaw[i].equals("sqr") || splitLaw[i].equals("log10") 
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		if (paramsOnly && !((String) type.getSelectedItem()).equals("Original")) {
		  ...
		  if (((String) type.getSelectedItem()).equals("Sweep")) {
		    ...
		    double stopVal = Double.parseDouble(stop.getText().trim());
		    double stepVal = Double.parseDouble(step.getText().trim());
		    param += "Sweep (" + startVal + "," + stopVal + ","
		      + stepVal + "," + level.getSelectedItem() + ")";
		  }
		  else {
		    param += "Custom " + val;
		  }
	...
	private void reacParametersEditor(String option) {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
	    ...
	    if (!error) {
		  ...
		  JOptionPane.showMessageDialog(biosim.frame(),
						"ID is not unique.", "ID Not Unique",
						JOptionPane.ERROR_MESSAGE);
		  error = true;
	      }
	    }
		...
		if (option.equals("OK")) {
		  ...
		  if (paramsOnly) {
		    ...
		    if (!((String) type.getSelectedItem()).equals("Original")) {
		      ...
		      reacts = Buttons.getList(reacts, reactions);
		      reactions.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		      reacts[index1] = ((String) reactions.getSelectedValue()).split(" ")[0] + " Modified";
		      sort(reacts);
		      reactions.setListData(reacts);
		      reactions.setSelectedIndex(index1);
		    }
	...
	public void productsEditor(String option) {
	  ...
	  if (option.equals("OK")) {
	    ...
	    if (product.isSetStoichiometryMath()) {
	      ...
	      productStoiciometry.setText("" + myFormulaToString(product.getStoichiometryMath().getMath()));
	    } else {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		JOptionPane.showMessageDialog(biosim.frame(),
					      "The stoichiometry must be a real number.",
		...
		if (productStoiciometry.getText().trim().equals("")) {
		  ...
		  ArrayList<String> invalidVars = getInvalidVariables(productStoiciometry.getText().trim(),true,"",false);
		  if (invalidVars.size() > 0) {
		    String invalid = "";
		    for (int i = 0; i < invalidVars.size(); i++) {
		      if (i == invalidVars.size() - 1) {
		  ...
		  if (!error) {
		    if (myParseFormula(productStoiciometry.getText().trim()).isBoolean()) {
		      JOptionPane.showMessageDialog(biosim.frame(), "Stoichiometry math must evaluate to a number.",
						    "Number Expected", JOptionPane.ERROR_MESSAGE);
		      error = true;
		    }	    
		  }
		...
		if (stoiciLabel.getSelectedItem().equals("Stoichiometry")) {
		  ...
		  StoichiometryMath sm = new StoichiometryMath(myParseFormula(productStoiciometry.getText().trim()));
	...
	public void reactantsEditor(String option) {
	  ...
	  if (option.equals("OK")) {
	    ...
	    if (reactant.isSetStoichiometryMath()) {
	      stoiciLabel.setSelectedItem("Stoichiometry Math");
	      reactantStoiciometry.setText("" + myFormulaToString(reactant.getStoichiometryMath().getMath()));
	    } else {
	  ...
	  while (error && value == JOptionPane.YES_OPTION) {
		...
		if (reactantStoiciometry.getText().trim().equals("")) {
		  ...
		  ArrayList<String> invalidVars = getInvalidVariables(reactantStoiciometry.getText().trim(),true,"",false);
		  if (invalidVars.size() > 0) {
		    String invalid = "";
		    for (int i = 0; i < invalidVars.size(); i++) {
		      if (i == invalidVars.size() - 1) {
		  ...
		  if (!error) {
		    error = checkNumFunctionArguments(myParseFormula(reactantStoiciometry.getText().trim()));
		  }
		  if (!error) {
		...
		if (stoiciLabel.getSelectedItem().equals("Stoichiometry")) {
		  ...
		  StoichiometryMath sm = new StoichiometryMath(myParseFormula(reactantStoiciometry.getText().trim()));
	...
	public ASTNode myParseFormula(String formula) {
	  ...
	  if (mathFormula==null) return null;
	...
	public boolean checkNumFunctionArguments(ASTNode node) {
	  ...
	  switch (node.getType()) {
		...
		JOptionPane.showMessageDialog(biosim.frame(), "Expected 1 argument for " + node.getName() + 
					      " but found " + node.getNumChildren() + ".", 
					      "Number of Arguments Incorrect", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      if ( node.getChild(0).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument for " + node.getName() + " function must evaluate to a number.",
					      "Number Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      break;
		...
		return true;
	      ...
	      if ( !node.getChild(0).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument for not function must be of type Boolean.",
		...
		return true;
	      ...
	      if ( !node.getChild(0).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument 1 for " + node.getName() + " function is not of type Boolean.",
					      "Boolean Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      if ( !node.getChild(1).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument 2 for " + node.getName() + " function is not of type Boolean.",
					      "Boolean Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      break;
	    case libsbml.AST_PLUS:
	      if ( node.getChild(0).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument 1 for + operator must evaluate to a number.",
					      "Number Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      if ( node.getChild(1).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument 2 for + operator must evaluate to a number.",
					      "Number Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      break;
		...
		return true;
	      ...
	      if ( node.getChild(0).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument 1 for " + node.getName() + " function must evaluate to a number.",
					      "Number Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      if ( node.getChild(1).isBoolean() ) {
		JOptionPane.showMessageDialog(biosim.frame(), "Argument 2 for " + node.getName() + " function must evaluate to a number.",
					      "Number Expected", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      break;
		...
		JOptionPane.showMessageDialog(biosim.frame(), "Arguments for " + node.getName() + " function must either both be numbers or Booleans.",
	    ...
	    case libsbml.AST_FUNCTION_PIECEWISE:
	      if (node.getNumChildren() < 1) {
		JOptionPane.showMessageDialog(biosim.frame(), "Piecewise function requires at least 1 argument.",
					      "Number of Arguments Incorrect", JOptionPane.ERROR_MESSAGE);
		return true;
	      }
	      for (int i = 1; i < node.getNumChildren(); i+=2) {
		if ( !node.getChild(i).isBoolean() ) {
		  JOptionPane.showMessageDialog(biosim.frame(), "Even arguments of piecewise function must be of type Boolean.",
						"Boolean Expected", JOptionPane.ERROR_MESSAGE);
		  return true;
		}
	      }
		...
		if ( node.getChild(i).isBoolean() ) {
		  if ( pieceType == 2 ) {
		    JOptionPane.showMessageDialog(biosim.frame(), "All odd arguments of a piecewise function must agree.",
		...
		if (((FunctionDefinition) sbml.get(i)).getId().equals(node.getName())) {
		  ...
		  if (numArgs != node.getNumChildren()) {
		    JOptionPane.showMessageDialog(biosim.frame(), "Expected " + numArgs + " argument(s) for " + node.getName() + 
						  " but found " + node.getNumChildren() + ".", 
						  "Number of Arguments Incorrect", JOptionPane.ERROR_MESSAGE);
		    return true;
		  }
	    ...
	    case libsbml.AST_NAME:
	      if (node.getName().equals("abs") 
		  || node.getName().equals("arccos") || node.getName().equals("arccosh")
		  || node.getName().equals("arcsin") || node.getName().equals("arcsinh") 
		  || node.getName().equals("arctan") || node.getName().equals("arctanh")
		  || node.getName().equals("arccot") || node.getName().equals("arccoth")
		  || node.getName().equals("arccsc") || node.getName().equals("arccsch")
		  || node.getName().equals("arcsec") || node.getName().equals("arcsech")
		  || node.getName().equals("acos") || node.getName().equals("acosh")
		  || node.getName().equals("asin") || node.getName().equals("asinh") 
		  || node.getName().equals("atan") || node.getName().equals("atanh")
		  || node.getName().equals("acot") || node.getName().equals("acoth")
		  || node.getName().equals("acsc") || node.getName().equals("acsch")
		  || node.getName().equals("asec") || node.getName().equals("asech")
		  || node.getName().equals("cos") || node.getName().equals("cosh") 
		  || node.getName().equals("cot") || node.getName().equals("coth")
		  || node.getName().equals("csc") || node.getName().equals("csch")
		  || node.getName().equals("ceil") || node.getName().equals("factorial")
		  || node.getName().equals("exp") || node.getName().equals("floor")
		  || node.getName().equals("ln") || node.getName().equals("log") 
		  || node.getName().equals("sqr") || node.getName().equals("log10") 
		  || node.getName().equals("sqrt")
	...
	public boolean checkRateRuleUnits(Rule rule) {
	  ...
	  if (rule.containsUndeclaredUnits()) {
	    JOptionPane.showMessageDialog(biosim.frame(), "Rate rule contains literals numbers or parameters with undeclared units.\n" +
		...
		Unit recTimeUnit = new Unit(timeUnit.getKind(),timeUnit.getExponent()*(-1),timeUnit.getScale(),timeUnit.getMultiplier());
	    ...
	    if (!libsbml.areEquivalent(unitDef,unitDefVar)) {
	      JOptionPane.showMessageDialog(biosim.frame(), "Units on the left and right-hand side of the rate rule do not agree.",
	...
	public boolean checkAssignmentRuleUnits(Rule rule) {
 	  document.getModel().populateListFormulaUnitsData();
	  if (rule.containsUndeclaredUnits()) {
	    JOptionPane.showMessageDialog(biosim.frame(), "Assignment rule contains literals numbers or parameters with undeclared units.\n" +
					  "Therefore, it is not possible to completely verify the consistency of the units.",
					  "Contains Undeclared Units", JOptionPane.WARNING_MESSAGE);
	    return false;
	  } else {
	    ...
	    if (!libsbml.areEquivalent(unitDef,unitDefVar)) {
	      JOptionPane.showMessageDialog(biosim.frame(), "Units on the left and right-hand side of the assignment rule do not agree.",
					    "Units Do Not Match", JOptionPane.ERROR_MESSAGE);
	      return true;
	    }
	  }
	...
	public boolean checkInitialAssignmentUnits(InitialAssignment init) {
	  ...
	  if (init.containsUndeclaredUnits()) {
	    JOptionPane.showMessageDialog(biosim.frame(), "Initial assignment contains literals numbers or parameters with undeclared units.\n" +
					  "Therefore, it is not possible to completely verify the consistency of the units.",
					  "Contains Undeclared Units", JOptionPane.WARNING_MESSAGE);
	    return false;
	  } else {
	    ...
	    if (!libsbml.areEquivalent(unitDef,unitDefVar)) {
	      JOptionPane.showMessageDialog(biosim.frame(), "Units on the left and right-hand side of the initial assignment do not agree.",
					    "Units Do Not Match", JOptionPane.ERROR_MESSAGE);
	      return true;
	    }
...
// 	    }
	...
	public boolean checkEventAssignmentUnits(EventAssignment assign) {
	  ...
	  if (assign.containsUndeclaredUnits()) {
	    JOptionPane.showMessageDialog(biosim.frame(), "Event assignment to " + assign.getVariable() + " contains literals numbers or parameters with undeclared units.\n" +
					  "Therefore, it is not possible to completely verify the consistency of the units.",
					  "Contains Undeclared Units", JOptionPane.WARNING_MESSAGE);
	    return false;
	  } else {
	    ...
	    if (!libsbml.areEquivalent(unitDef,unitDefVar)) {
	      JOptionPane.showMessageDialog(biosim.frame(), "Units on the left and right-hand side for the event assignment " + assign.getVariable() + " do not agree.",
					    "Units Do Not Match", JOptionPane.ERROR_MESSAGE);
	      return true;
	    }
	  }
...
// 	  }
	...
	public boolean checkKineticLawUnits(KineticLaw law) {
	  ...
	  if (law.containsUndeclaredUnits()) {
	    JOptionPane.showMessageDialog(biosim.frame(), "Kinetic law contains literals numbers or parameters with undeclared units.\n" +
					  "Therefore, it is not possible to completely verify the consistency of the units.",
					  "Contains Undeclared Units", JOptionPane.WARNING_MESSAGE);
	    return false;
	  } else {
	    ...
	    if (!libsbml.areEquivalent(unitDef,unitDefLaw)) {
	      JOptionPane.showMessageDialog(biosim.frame(), "Kinetic law units should be substance / time.",
					    "Units Do Not Match", JOptionPane.ERROR_MESSAGE);
	      return true;
	    }
	  }
