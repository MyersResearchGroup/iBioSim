		...
		try
		{
					...
					&& (monteCarlo.isSelected() || xhtml.isSelected() || dot.isSelected()))
			{
				...
				if (abstraction.isSelected())
				{
					...
					t1.convertLPN2SBML(root + File.separator + simName + File.separator + lpnName
							+ ".temp", prop);
				}
				else
				{
			...
			else if (fba.isSelected())
			{
				...
				FluxBalanceAnalysis fluxBalanceAnalysis = new FluxBalanceAnalysis(directory
						+ File.separator, theFile, absError);
				...
				if (sbmlName != null && !sbmlName.trim().equals(""))
				{
					...
					if (modelFile.contains(".lpn"))
					{
						...
						if (abstraction.isSelected())
						{
							...
							t1.convertLPN2SBML(root + File.separator + simName + File.separator
					...
					else if (modelEditor != null && nary.isSelected())
					{
						...
						if (bioModel.flattenModel(true) != null)
						{
							...
							{
								LhpnFile lhpnFile = new LhpnFile();
								lhpnFile.load(root + File.separator + simName + File.separator
										+ lpnName);
								Abstraction abst = new Abstraction(lhpnFile, abstPane);
								abst.abstractSTG(false);
					...
					else
					{
								...
								&& (abstraction.isSelected() || nary.isSelected()))
						{
							log.addText("Executing:\nreb2sac --target.encoding=sbml --out=" + ".."
									+ File.separator + sbmlName + " " + filename + "\n");
							logFile.write("Executing:\nreb2sac --target.encoding=sbml --out="
									+ ".." + File.separator + sbmlName + " " + filename + "\n\n");
							time1 = System.nanoTime();
							reb2sac = exec.exec("reb2sac --target.encoding=sbml --out=" + ".."
									+ File.separator + sbmlName + " " + theFile, null, work);
						}
						else
						{
			...
			else if (dot.isSelected())
			{
				if (nary.isSelected() && modelEditor != null)
				{
					...
					lhpnFile.load(directory + File.separator
							+ theFile.replace(".sbml", "").replace(".xml", "") + ".lpn");
					lhpnFile.printDot(directory + File.separator
				...
				else if (modelFile.contains(".lpn"))
				{
					...
					if (abstraction.isSelected())
					{
						Abstraction abst = new Abstraction(lhpnFile, abstPane);
						abst.abstractSTG(false);
						abst.printDot(root + File.separator + simName + File.separator
								+ modelFile.replace(".lpn", ".dot"));
					}
					else
					{
						lhpnFile.printDot(root + File.separator + simName + File.separator
								+ modelFile.replace(".lpn", ".dot"));
					}
					time1 = System.nanoTime();
					exitValue = 0;
			...
			else
			{
							...
							if (!sg.getStop())
							{
								...
								if (simrep != null)
								{
									FileOutputStream simrepstream = new FileOutputStream(new File(
											directory + File.separator + "sim-rep.txt"));
								...
								if (!sg.getStop())
								{
									...
									if (simrep != null)
									{
										FileOutputStream simrepstream = new FileOutputStream(
												new File(directory + File.separator + "sim-rep.txt"));
										simrepstream.write((simrep).getBytes());
										simrepstream.close();
									}
									Object[] options = { "Yes", "No" };
									...
									if (sg.outputTSD(directory + File.separator
				...
				else
				{
					...
					else if (sim.equals("SSA-CR"))
					{
						...
						dynSim.simulate(SBMLFileName, root, outDir + File.separator, timeLimit,
				...
				else if (sbml.isSelected())
				{
					if (sbmlName != null && !sbmlName.trim().equals(""))
					{
						if (!gui.updateOpenModelEditor(sbmlName))
						{
							try
							{
								ModelEditor gcm = new ModelEditor(root + File.separator, sbmlName,
							...
							if (simTab.getComponentAt(i).getName().equals("TSD Graph"))
							{
								if (simTab.getComponentAt(i) instanceof Graph)
								{
									...
									if (outputTerm)
									{
										...
										if (new File(directory + File.separator + "sim-rep.txt")
												.exists())
										{
											try
											{
										...
										probData.outputTSD(directory + File.separator
												+ "term-time.tsd");
