...
public class HierarchicalSimWrapper
{
	...
	public static void main(String[] args)
	{
		 ...
		 * 
		 * 
		 * if (System.getProperty("mrj.version") != null) varname =
		 * "DYLD_LIBRARY_PATH"; // We're on a Mac. else varname =
		 * "LD_LIBRARY_PATH"; // We're not on a Mac.
		 * 
		 * try { System.loadLibrary("sbmlj"); // For extra safety, check that
		 * the jar file is in the // classpath.
		 * Class.forName("org.sbml.jsbml.JSBML"); } catch (UnsatisfiedLinkError
		 * e) { e.printStackTrace();
		 * System.err.println("Error: could not link with the libSBML library."
		 * + "  It is likely\nyour " + varname +
		 * " environment variable does not include\nthe" +
		 * " directory containing the libsbml library file."); System.exit(1); }
		 * catch (ClassNotFoundException e) {
		 * System.err.println("Error: unable to load the file libsbmlj.jar." +
		 * "  It is likely\nyour " + varname + " environment" +
		 * " variable or CLASSPATH variable\ndoes not include" +
		 * " the directory containing the libsbmlj.jar file."); System.exit(1);
		 * } catch (SecurityException e) {
		 * System.err.println("Could not load the libSBML library files due to a"
		 * + " security exception."); System.exit(1); }
		 */
		...
		if (args.length < 1)
		{
			 ...
			 * args = new String[3];
			 * 
			 * args[0] = "C:\\sbml-testsuite\\cases\\semantic\\"; args[1] =
			 * "00001"; args[2] = "C:\\Users\\Leandro\\ibiosim-testsuite";
			 */
	 ...
	 * 
	 * File f = new File(filename); Properties properties = new Properties();
	 * FileInputStream in;
	 * 
	 * try { in = new FileInputStream(f); properties.load(in); } catch
	 * (Exception e) { e.printStackTrace(); }
	 * 
	 * String simMethod = properties.getProperty("reb2sac.simulation.method");
	 * String prefix = "monte.carlo.";
	 * 
	 * if (simMethod.equals("ODE")) prefix = "ode.";
	 * 
	 * if (properties.containsKey(prefix + "simulation.time.limit")) timeLimit =
	 * Double.valueOf(properties.getProperty(prefix + "simulation.time.limit"));
	 * if (properties.containsKey(prefix + "simulation.time.step")) maxTimeStep
	 * = Double.valueOf(properties.getProperty(prefix +
	 * "simulation.time.step")); if (properties.containsKey(prefix +
	 * "simulation.print.interval")) printInterval =
	 * Double.valueOf(properties.getProperty(prefix +
	 * "simulation.print.interval")); if
	 * (properties.containsKey("monte.carlo.simulation.runs")) runs =
	 * Integer.valueOf(properties.getProperty("monte.carlo.simulation.runs"));
	 * if (properties.containsKey("reb2sac.generate.statistics")) genStats =
	 * Boolean.valueOf(properties.getProperty("reb2sac.generate.statistics"));
	 * if (properties.containsKey("monte.carlo.simulation.random.seed"))
	 * randomSeed =
	 * Long.valueOf(properties.getProperty("monte.carlo.simulation.random.seed"
	 * )); if (properties.containsKey("simulation.printer.tracking.quantity"))
	 * quantityType =
	 * properties.getProperty("simulation.printer.tracking.quantity"); if
	 * (properties.containsKey("monte.carlo.simulation.min.time.step"))
	 * minTimeStep =
	 * Double.valueOf(properties.getProperty("monte.carlo.simulation.min.time.step"
	 * )); if
	 * (properties.containsKey("reb2sac.diffusion.stoichiometry.amplification.value"
	 * )) stoichAmpValue = Double.valueOf(properties.getProperty(
	 * "reb2sac.diffusion.stoichiometry.amplification.value")); if
	 * (properties.containsKey("selected.simulator")) selectedSimulator =
	 * properties.getProperty("selected.simulator"); //relativeError =
	 * Double.valueOf(properties.getProperty("relative")); if
	 * (properties.containsKey("ode.simulation.absolute.error")) absoluteError =
	 * Double.valueOf(properties.getProperty("ode.simulation.absolute.error"));
	 * if (properties.containsKey(prefix + "simulation.number.steps")) numSteps
	 * = Integer.valueOf(properties.getProperty(prefix +
	 * "simulation.number.steps"));
	 * 
	 * int intSpecies = 1;
	 * 
	 * while (properties.containsKey("reb2sac.interesting.species." +
	 * intSpecies)) {
	 * 
	 * interestingSpecies.add(properties.getProperty("reb2sac.interesting.species."
	 * + intSpecies)); ++intSpecies; } }
	 */
