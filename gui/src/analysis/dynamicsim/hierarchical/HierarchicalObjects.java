...
public abstract class HierarchicalObjects extends HierarchicalSimState {
	...
	protected class HierarchicalEventComparator implements Comparator<HierarchicalEventToFire> {
		...
		public int compare(HierarchicalEventToFire event1, HierarchicalEventToFire event2) {
			...
			if (event1.getFireTime() > event2.getFireTime())
				...
				if (evaluateExpressionRecursive(state1, state1.getEventToPriorityMap().get(event1.getEventID()), false, getCurrentTime(), null, null) >  
				evaluateExpressionRecursive(state2, state2.getEventToPriorityMap().get(event2.getEventID()), false, getCurrentTime(), null, null))
					return -1;
				else if ( evaluateExpressionRecursive(state1, state1.getEventToPriorityMap().get(event1.getEventID()), false, getCurrentTime(), null, null) <  
						evaluateExpressionRecursive(state2, state2.getEventToPriorityMap().get(event2.getEventID()), false, getCurrentTime(), null, null))
					return 1;
				else {
					if ((Math.random() * 100) > 50) {
	...
	protected class ModelState extends ArraysState
	{
		...
		public ModelState(HashMap<String, Model> models, String bioModel,
				String submodelID)
		{
		 ...
		 */
		public HashSet<HierarchicalStringPair> getSpeciesToReplacement(
				String species)
				{
			if (getSpeciesToReplacement().get(species) == null)
			{
				getSpeciesToReplacement().put(species,
		...
		public double getVariableToValue(HashMap<String, Double> replacements,
				String variable)
		{
		...
		public void setvariableToValueMap(HashMap<String, Double> replacements,
	...
	private double evaluateBoolean(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
		switch (node.getType()) {
		...
		case  LOGICAL_NOT:
			return HierarchicalUtilities.getDoubleFromBoolean(!(HierarchicalUtilities.getBooleanFromDouble(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap))));
		...
		case LOGICAL_AND: {
			...
			for (int childIter = 0; childIter < node.getChildCount(); ++childIter)
				andResult = andResult && HierarchicalUtilities.getBooleanFromDouble(evaluateExpressionRecursive(modelstate, node.getChild(childIter), evaluateState, t, y, variableToIndexMap));
		...
		case LOGICAL_OR: {
			...
			for (int childIter = 0; childIter < node.getChildCount(); ++childIter)
				orResult = orResult || HierarchicalUtilities.getBooleanFromDouble(evaluateExpressionRecursive(modelstate, node.getChild(childIter), evaluateState, t, y, variableToIndexMap));
		...
		case LOGICAL_XOR: {
			...
			boolean xorResult = (node.getChildCount()==0) ? false:
				HierarchicalUtilities.getBooleanFromDouble(evaluateExpressionRecursive(modelstate, node.getChild(0), evaluateState, t, y, variableToIndexMap));

			for (int childIter = 1; childIter < node.getChildCount(); ++childIter)
				xorResult = xorResult ^ HierarchicalUtilities.getBooleanFromDouble(evaluateExpressionRecursive(modelstate, node.getChild(childIter), evaluateState, t, y, variableToIndexMap));
		...
		case RELATIONAL_EQ:
			return HierarchicalUtilities.getDoubleFromBoolean(
					evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap) == evaluateExpressionRecursive(modelstate, node.getRightChild(), evaluateState, t, y, variableToIndexMap));
	...
	private double evaluateConstant(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
	...
	protected double evaluateExpressionRecursive(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap) {
	...
	private double evaluateFunction(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
		switch(node.getType())
		{
		case FUNCTION: {
			...
			if (nodeName.equals("uniform")) {
				...
				return prng.nextExponential(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap), 1);
			}
			else if (nodeName.equals("gamma")) {

				return prng.nextGamma(1, evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap), 
				...
				return prng.nextChiSquare((int) evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap));
			}
			else if (nodeName.equals("lognormal")) {

				return prng.nextLogNormal(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap), 
						evaluateExpressionRecursive(modelstate, node.getRightChild(), evaluateState, t, y, variableToIndexMap));
			}
				...
				return prng.nextLorentzian(0, evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap));
			}
			else if (nodeName.equals("poisson")) {

				return prng.nextPoissonian(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap));
			}
			else if (nodeName.equals("binomial")) {

				return prng.nextBinomial(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap),
				...
				return prng.nextBinomial(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap), 1);
			}
			else if (nodeName.equals("normal")) {

				return prng.nextGaussian(evaluateExpressionRecursive(modelstate, node.getLeftChild(), evaluateState, t, y, variableToIndexMap),
						evaluateExpressionRecursive(modelstate, node.getRightChild(), evaluateState, t, y, variableToIndexMap));	
			}
		...
		case FUNCTION_PIECEWISE: {
			...
			for (int childIter = 0; childIter < node.getChildCount(); childIter += 3) {
				...
				if ((childIter + 1) < node.getChildCount() && 
						HierarchicalUtilities.getBooleanFromDouble(evaluateExpressionRecursive(modelstate, node.getChild(childIter + 1), evaluateState, t, y, variableToIndexMap))) {
		...
		case FUNCTION_POWER:
		{
			...
			return (FastMath.pow(evaluateExpressionRecursive(modelstate, leftChild, evaluateState, t, y, variableToIndexMap), evaluateExpressionRecursive(modelstate, rightChild, evaluateState, t, y, variableToIndexMap)));
		}
	...
	private double evaluateInteger(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
		return node.getInteger();
	}

	private double evaluateName(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
	...
	private double evaluateOperator(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
		switch (node.getType()) {
		case PLUS: {
		...
		case DIVIDE:
		{
			...
			return (evaluateExpressionRecursive(modelstate, leftChild, evaluateState, t, y, variableToIndexMap) / evaluateExpressionRecursive(modelstate, rightChild, evaluateState, t, y, variableToIndexMap));
	...
	private double evaluateReal(ModelState modelstate, ASTNode node, boolean evaluateState, double t, double[] y, HashMap<String, Integer> variableToIndexMap)
	{
		return node.getReal();
	}
	...
	protected ASTNode inlineFormula(ModelState modelstate, ASTNode formula) {
		...
		if (formula.isFunction()
			...
			if (getIbiosimFunctionDefinitions().contains(
					formula.getName()))
				return formula;
			...
			if (modelstate.getVariableToIsConstantMap().containsKey(variable) && modelstate.getVariableToIsConstantMap().get(variable) == false
				...
				if (modelstate.getSpeciesToHasOnlySubstanceUnitsMap().containsKey(variable) &&
					...
					modelstate.setvariableToValueMap(replacements, variable, 
							evaluateExpressionRecursive(modelstate, assignmentRule.getMath(), false, getCurrentTime(), null, null) * 
							modelstate.getVariableToValue(replacements, modelstate.getSpeciesToCompartmentNameMap().get(variable)));
				}
				else {
					modelstate.setvariableToValueMap(replacements, variable, evaluateExpressionRecursive(modelstate, assignmentRule.getMath(), false, getCurrentTime(), null, null));
				}
