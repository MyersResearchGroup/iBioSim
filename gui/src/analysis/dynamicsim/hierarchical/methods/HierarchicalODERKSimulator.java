...
public class HierarchicalODERKSimulator  extends HierarchicalSimulationFunctions{
	...
	public HierarchicalODERKSimulator(String SBMLFileName, String rootDirectory, String outputDirectory, double timeLimit, double maxTimeStep, long randomSeed,
	...
	public void simulate() {
		...
		while (getCurrentTime() <= getTimeLimit()&& !isCancelFlag() && !isConstraintFlag()) 
		{
			...
			for(DiffEquations eq : functions)
			{
				...
				if(getCurrentTime()  < nextEndTime && Math.abs(getCurrentTime()  - nextEndTime) > 1e-6)
				{
					odecalc.integrate(eq, getCurrentTime() , eq.state.values, nextEndTime, eq.state.values);
					//handleEvents();
					//fireEvent(eq, modelstate);
	...
	private void updateValuesArray()
	{
		...
		for(DiffEquations eq : functions)
		{
			for (String variable : eq.state.variables) 
			{
				...
				eq.state.values[index] = eq.state.modelstate.getVariableToValue(getReplacements(),variable);
			}
		}
	}
			...
			if (sizeAfter != sizeBefore)
			{
				for (String var : vars)
				{
					...
					if (modelstate.getSpeciesToHasOnlySubstanceUnitsMap().containsKey(var) &&
						...
						eq.state.values[index] = modelstate.getVariableToValue(getReplacements(), var) * 
						modelstate.getVariableToValueMap().get(modelstate.getSpeciesToCompartmentNameMap().get(var));
					else
						eq.state.values[index] = modelstate.getVariableToValue(getReplacements(), var);	
				}
			}
	...
	private class DiffEquations implements FirstOrderDifferentialEquations
	{
		...
		public void computeDerivatives(double t, double[] y, double[] currValueChanges)
			...
			for (int i = 0; i < currValueChanges.length - 1; i++) {
				...
				if ((state.modelstate.getSpeciesIDSet().contains(currentVar) && 
						state.modelstate.getSpeciesToIsBoundaryConditionMap().get(currentVar) == false) &&
				...
				if (state.modelstate.getVariableToIsInAssignmentRuleMap() != null && state.modelstate.getVariableToIsInAssignmentRuleMap().containsKey(currentVar) &&
						state.modelstate.getVariableToValueMap().contains(currentVar) && 
			...
			//updatePropensities(performRateRules(getTopmodel(), currentTime), "getTopmodel()");
			...
			if (affectedAssignmentRuleSet.size() > 0) {
				...
				for (String affectedVariable : affectedVariables) 
				{
					...
					currValueChanges[index] = state.modelstate.getVariableToValue(getReplacements(),affectedVariable) - y[index];
	...
	protected class VariableState
	{
		...
		protected VariableState(ModelState modelstate)
		{
			...
			for (String reaction : modelstate.getReactionToFormulaMap().keySet()) 
			{
				...
				HashSet<HierarchicalStringDoublePair> reactantAndStoichiometrySet = modelstate.getReactionToReactantStoichiometrySetMap().get(reaction);
				HashSet<HierarchicalStringDoublePair> speciesAndStoichiometrySet = modelstate.getReactionToSpeciesAndStoichiometrySetMap().get(reaction);
				HashSet<HierarchicalStringPair> nonConstantStoichiometrySet = modelstate.getReactionToNonconstantStoichiometriesSetMap().get(reaction);
