...
public class SpeciesPanel extends JPanel implements ActionListener {
	...
	public SpeciesPanel(String selected, PropertyList speciesList, PropertyList componentsList, 
		...
		this.fields = new HashMap<String, PropertyField>();
		
//		constructor(selected, speciesList, componentsList, bioModel, paramsOnly, refGCM, modelEditor, inTab);
		loadSpeciesEditor(inTab);
	}
	
	
	/**
	 * Add the given component to the Species Editor panel in a grid formatted fashion.
	 * @param speciesEditorPanel - The panel where the component will be placed on.
	 * @param speciesEditorField - The field or value to add to the species editor.
	 * @param rowIndex - The row's index value where the speciesEditorField will be placed in. rowIndex must be greater than or equal to 0. The value will grow from top to bottom.
	 * @param colIndex - The col's index value where the speciesEditorField will be placed in. colIndex must be greater than or equal to 0. The value will grow from left to right.
	 * @param mergeColNums - The number of columns to merge the cell with for the GridBagLayout
	 * @param xPadding - Indicate the amount of internal padding the component will have in the editor panel for the GridBagLayout.
	 * @param compInset - Indicate the external padding for the given component in the editor panel for the GridBagLayout.
	 */
	private void addSpeciesEditor_Field(JPanel speciesEditorPanel, java.awt.Component speciesEditorField, 
			int rowIndex, int colIndex, int mergeColNums, int xPadding, java.awt.Insets compInset)
	{
		...
		speciesEditorPanel.add(speciesEditorField, c);
	}
	
	
	private void loadSpeciesEditor(boolean inTab)
	{
		JPanel speciesEditorPanel = new JPanel(new GridBagLayout());
		JPanel speciesPrimaryPanel = new JPanel(new GridBagLayout());
		
		int labelRow = 0, valueRow = 0, labelCol = 0, valueCol = 1;
		int xPrimaryFieldPadding = 245, xNoPadding = 0;
		int mergeNoCol = 0, merge1Col = 1, merge2Cols = 2, merge3Cols = 3;
		java.awt.Insets noInset = null;
		...
		String speciesIDText = species.getId() + dimInID;
		PropertyField field = new PropertyField(GlobalConstants.ID + ":", speciesIDText, null, null, Utility.IDDimString, paramsOnly, "default", false);
		fields.put(GlobalConstants.ID, field);
		JLabel speciesIDLabel = new JLabel(GlobalConstants.ID + ":", JLabel.RIGHT);
		JTextField speciesID = new JTextField(speciesIDText);
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesIDLabel, labelRow++, labelCol, mergeNoCol, xPrimaryFieldPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesID, valueRow++, valueCol, merge1Col, xPrimaryFieldPadding, noInset); 

		// Name field
		String speciesNameText = species.getName();
		field = new PropertyField(GlobalConstants.NAME, speciesNameText, null, null, Utility.NAMEstring, paramsOnly, 
				"default", false);
		fields.put(GlobalConstants.NAME, field);
		JLabel speciesNameLabel = new JLabel(GlobalConstants.NAME + ":", JLabel.RIGHT);
		JTextField speciesName = new JTextField(speciesNameText);
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesNameLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesName, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset);
		
		// Port Type field
		JLabel speciesPortLabel = new JLabel(GlobalConstants.PORTTYPE + ":", JLabel.RIGHT);
		typeBox = new JComboBox(types);
		typeBox.addActionListener(this);
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesPortLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, typeBox, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset);  
		
		// SBO Term field
		JLabel speciesSBOLabel = new JLabel(GlobalConstants.SBOTERM, JLabel.RIGHT);
		...
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesSBOLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, SBOTerms, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
				
		// compartment field
		JLabel speciesCompartmentLabel = new JLabel("Compartment" + ":", JLabel.RIGHT);
		compartBox = MySpecies.createCompartmentChoices(bioModel);
		compartBox.addActionListener(this);
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesCompartmentLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, compartBox, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
		
		// indices field
		JLabel speciesCompIndicesLabel = new JLabel("Compartment Indices" + ":", JLabel.RIGHT);
		...
		addSpeciesEditor_Field(speciesPrimaryPanel, speciesCompIndicesLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, iIndex, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
		...
		if (paramsOnly) 
		{
			...
			addSpeciesEditor_Field(speciesEditorPanel, field, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		}
		else 
		{
			JLabel initConLabel = new JLabel("Initial Amount/Concentration" + ":", JLabel.RIGHT);
			...
			addSpeciesEditor_Field(speciesPrimaryPanel, initConLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
			addSpeciesEditor_Field(speciesPrimaryPanel, initialField, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
		}
		...
		addSpeciesEditor_Field(speciesPrimaryPanel, unitLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, unitsBox, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
		
		// Conversion factor field
		JLabel conFLabel = new JLabel("Conversion Factor" + ":", JLabel.RIGHT);
		...
		addSpeciesEditor_Field(speciesPrimaryPanel, conFLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, convBox, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 

		
		JLabel conFIndLabel = new JLabel("Conversion Factor Indices" + ":", JLabel.RIGHT);
		String cfreshIndex = SBMLutilities.getIndicesString(species, "conversion");
		conviIndex.setText(cfreshIndex);
		addSpeciesEditor_Field(speciesPrimaryPanel, conFIndLabel, labelRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		addSpeciesEditor_Field(speciesPrimaryPanel, conviIndex, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
		
		//Add the primary species field to the main species editor
		labelRow = 0; valueRow = 0; labelCol = 0; valueCol = 1 ;
		addSpeciesEditor_Field(speciesEditorPanel, speciesPrimaryPanel, valueRow++, valueCol, mergeNoCol, xNoPadding, noInset); 
		...
		checkBoxPanel.add(tempPanel);
		addSpeciesEditor_Field(speciesEditorPanel, checkBoxPanel, valueRow++, labelCol, 4, xNoPadding, new java.awt.Insets(0,100,0,0)); 
		...
		if (paramsOnly) 
		{
			...
			addSpeciesEditor_Field(speciesEditorPanel, tempPanel, valueRow++, labelCol, mergeNoCol, xNoPadding, noInset); 
		}		
		...
		addSpeciesEditor_Field(speciesEditorPanel, field, valueRow++, labelCol, merge3Cols, xNoPadding, noInset); 
		...
		if (!paramsOnly) {
			...
			addSpeciesEditor_Field(speciesEditorPanel, sbolField, valueRow++, labelCol, 4, xNoPadding, noInset);
		...
		//Display the Species Editor
		this.add(speciesEditorPanel, BorderLayout.CENTER);
