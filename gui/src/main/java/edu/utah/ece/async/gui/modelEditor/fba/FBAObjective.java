 ...
 *  
 *******************************************************************************/
package edu.utah.ece.async.gui.modelEditor.fba;
...
public class FBAObjective extends JPanel implements ActionListener, MouseListener {
	...
	public FBAObjective(BioModel bioModel) {
		...
		edu.utah.ece.async.dataModels.biomodel.util.Utility.sort(objectiveStringArray);
		...
		while (error && value == JOptionPane.YES_OPTION) {
			...
			if (!error) {
				if (option.equals("OK")) {
					...
					edu.utah.ece.async.dataModels.biomodel.util.Utility.sort(listOfObjectives);
