...
public class SBOLAssemblyGraph {
	...
	private LinkedHashMap<String, SBOLAssemblyNode> synMap;
	private Set<SBOLAssemblyNode> startNodes = new HashSet<SBOLAssemblyNode>();
	private boolean containsSBOL = false;
	private boolean isLinear = false;
	
	public SBOLAssemblyGraph(BioModel biomodel) {
		synMap = new LinkedHashMap<String, SBOLAssemblyNode>(); // initialize map of model element meta IDs to synthesis nodes
...
//		}
		...
		parseSpeciesSbol(biomodel, sbmlModel, speciesMetaMap);
//		parsePromoterSbol(sbmlModel, speciesMetaMap);
		...
		if (containsSBOL) 
			connectMappedSynthesisNodes(paramInputMap, paramOutputMap);
		...
		for (long i = 0; i < compSBMLModel.getNumSubmodels(); i++) {
			...
			if (submodel.getNumDeletions() > 0) {
				...
				for (long j = 0; j < submodel.getNumDeletions(); j++) {
					String elementID = subCompSBMLModel.getPort(submodel.getDeletion(j).getPortRef()).getIdRef();
	...
	private void parseSpeciesSbol(BioModel biomodel, Model sbmlModel, HashMap<String, String> speciesMetaMap) {
		for (long i = 0; i < sbmlModel.getNumSpecies(); i++) {
			Species sbmlSpecies = sbmlModel.getSpecies(i);
//			if (!BioModel.isPromoterSpecies(sbmlSpecies)) {
			...
			SBOLAssemblyNode synNode = new SBOLAssemblyNode(sbmlSpecies.getMetaId(), sbolURIs);
			synMap.put(synNode.getID(), synNode);
...
//	}
	...
	private void parseReactionSbol(Model sbmlModel, HashMap<String, String> speciesMetaMap, HashMap<String, Set<String>> paramInputMap) {
		Set<String> globalParams = new HashSet<String>();
		for (long j = 0; j < sbmlModel.getNumParameters(); j++) {
			globalParams.add(sbmlModel.getParameter(j).getId());
		}
		for (long i = 0; i < sbmlModel.getNumReactions(); i++) {
			...
			SBOLAssemblyNode synNode = new SBOLAssemblyNode(sbmlReaction.getMetaId(), sbolURIs);
			synMap.put(synNode.getID(), synNode);
			...
			for (long j = 0; j < sbmlReaction.getNumReactants(); j++) {
				...
				if (speciesMetaMap.containsKey(reactantID)) {
					String reactantMetaID = speciesMetaMap.get(reactantID);
			...
			for (long j = 0; j < sbmlReaction.getNumModifiers(); j++) {
				...
				if (speciesMetaMap.containsKey(modifierID)) {
					String modifierMetaID = speciesMetaMap.get(modifierID);
			...
			for (long j = 0; j < sbmlReaction.getNumProducts(); j++) {
				...
				if (speciesMetaMap.containsKey(productID)) {
					String productMetaID = speciesMetaMap.get(productID);
					synNode.addNextNode(synMap.get(productMetaID));
				}
			}
...
//				}
				for (String input : parseInputHelper(kl.getMath())) 
					if (globalParams.contains(input)) {
	...
	private void parseRuleSbol(Model sbmlModel, HashMap<String, String> speciesMetaMap, 
		...
		for (long i = 0; i < sbmlModel.getNumRules(); i++) {
			...
			if (sbmlRule.isAssignment() || sbmlRule.isRate()) {
				...
				SBOLAssemblyNode synNode = new SBOLAssemblyNode(sbmlRule.getMetaId(), sbolURIs);
				synMap.put(synNode.getID(), synNode);
	...
	private void connectMappedSynthesisNodes(HashMap<String, Set<String>> paramInputMap, HashMap<String, Set<String>> paramOutputMap) {
...
//				synMap.get(infl.getInput()).addNextNode(synMap.get(complexId));
	...
	public boolean loadDNAComponents(SBOLFileManager fileManager) {
		boolean error = false;
		for (SBOLAssemblyNode synNode : getNodes()) {
...
//	}
	...
	private void cutNodesWithStartComponents(Set<String> startTypes) {
		int idIndex = 0;
		Set<SBOLAssemblyNode> cutNodes = new HashSet<SBOLAssemblyNode>();
		Set<SBOLAssemblyNode> deletionNodes = new HashSet<SBOLAssemblyNode>();
		for (SBOLAssemblyNode synNode : synMap.values()) {
			Set<SBOLAssemblyNode> emptyNodes = new HashSet<SBOLAssemblyNode>();
			for (SBOLAssemblyNode nextNode : synNode.getNextNodes()) {
				...
				if (cutIndices.size() > 0) {
					...
					for (int i = 0; i < cutIndices.size() - 1; i++) {
						SBOLAssemblyNode cutNode = new SBOLAssemblyNode("cut" + idIndex);
					...
					if (edgeComponents.size() > 0) {
						nextNode.setDNAComponents(edgeComponents);
						nextNode.setNextNodes(new LinkedList<SBOLAssemblyNode>());
					} else {
		...
		for (SBOLAssemblyNode cutNode : cutNodes)
			synMap.put(cutNode.getID(), cutNode);
		for (SBOLAssemblyNode deletionNode : deletionNodes)
	...
	private void cutCycles() {
		...
		for (SBOLAssemblyNode startNode : startNodes) {
			...
			while (currentNodes.size() > 0) {
				...
				Set<SBOLAssemblyNode> cutNodes = new HashSet<SBOLAssemblyNode>();
				for (SBOLAssemblyNode nextNode : currentNodes.get(0).getNextNodes())
	...
	public SBOLAssemblyNode getNode(String id) {
		return synMap.get(id);
	}
	
	public Set<SBOLAssemblyNode> getNodes() {
		return new HashSet<SBOLAssemblyNode>(synMap.values());
	}
