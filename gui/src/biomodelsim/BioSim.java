...
public class BioSim implements MouseListener, ActionListener {
	...
	private JMenuItem importHse; // The import handshaking extension menu item
	...
	public void preferences() {
		if (!lema) {
			...
			final JTextField concentration = new JTextField(biosimrc.get(
					"biosim.sim.concentration", ""));
	...
	public void actionPerformed(ActionEvent e) {
						...
						if (overwrite(root + separator + verName, verName)) {
							...
							try {
								FileInputStream in = new FileInputStream(new File(root + separator
										+ circuitFileNoPath));
								FileOutputStream out = new FileOutputStream(new File(root
										+ separator + verName.trim() + separator
										+ circuitFileNoPath));
								int read = in.read();
								while (read != -1) {
									out.write(read);
		...
		// if the delete popup menu is selected
			...
			String[] views = canDelete(tree.getFile().split(separator)[tree.getFile().split(
					separator).length - 1]);
			if (views.length == 0) {
				for (int i = 0; i < tab.getTabCount(); i++) {
					if (tab
		...
		// if the graph popup menu is selected on an sbml file
			...
			try {
				...
				run.createProperties(0, 1, 1, 1, tree.getFile()
						.substring(
								0,
								tree.getFile().length()
					...
					if (simName != null && !simName.trim().equals("")) {
						...
						if (!(IDpat.matcher(modelID).matches())) {
							...
							if (overwrite(root + separator + simName, simName)) {
								...
								addTab(f.getName(), new GCM2SBMLEditor(root + separator, f
										.getName(), this, log), "GCM Editor");
								refreshTree();
							}
						}
					}
				...
				if (lrnName != null && !lrnName.trim().equals("")) {
					...
					try {
						if (overwrite(root + separator + lrnName, lrnName)) {
							...
							lrnTab.addTab("Data Manager", new DataManager(root + separator
									+ lrnName, this));
							lrnTab.getComponentAt(lrnTab.getComponents().length - 1).setName(
									"Data Manager");
							lrnTab
									.addTab("Learn", new Learn(root + separator + lrnName, log,
											this));
							lrnTab.getComponentAt(lrnTab.getComponents().length - 1).setName(
									"Learn");
				...
				if (overwrite(root + separator + copy, copy)) {
					if (modelID != null) {
						...
						writer.writeSBML(document, root + separator + copy);
							...
							.getFile().substring(tree.getFile().length() - 4).equals(".vhd") || tree
							.getFile().substring(tree.getFile().length() - 4).equals(".csp") || tree
							.getFile().substring(tree.getFile().length() - 4).equals(".hse") || tree
							.getFile().substring(tree.getFile().length() - 4).equals(".unc") || tree
							...
							for (String ss : s) {
								if (ss.length() > 4
									...
									if (ss.substring(ss.length() - 4).equals(".pms")) {
										out = new FileOutputStream(new File(root + separator + copy
												+ separator + copy + ".sim"));
									}
				...
				if (!rename.equals("")) {
					...
					if (overwrite(root + separator + rename, rename)) {
						if (tree.getFile().length() >= 5
								...
								&& tree.getFile().substring(tree.getFile().length() - 4).equals(
										".rsg")) {
							String oldName = tree.getFile().split(separator)[tree.getFile().split(
									separator).length - 1];
						...
						if (new File(root + separator + rename).isDirectory()) {
							if (new File(root
								...
								new File(root
										...
										+ separator
										+ tree.getFile().split(separator)[tree.getFile().split(
												separator).length - 1] + ".sim").renameTo(new File(
								...
								new File(root
										...
										+ separator
										+ tree.getFile().split(separator)[tree.getFile().split(
												separator).length - 1] + ".pms").renameTo(new File(
										root + separator + rename + separator + rename + ".sim"));
							}
							if (new File(root
									+ separator
								...
								new File(root
										...
										+ separator
										+ tree.getFile().split(separator)[tree.getFile().split(
												separator).length - 1] + ".lrn").renameTo(new File(
							...
							if (new File(root
								...
								new File(root
										...
										+ separator
										+ tree.getFile().split(separator)[tree.getFile().split(
												separator).length - 1] + ".grf").renameTo(new File(
							...
							if (new File(root
								...
								new File(root
										...
										+ separator
										+ tree.getFile().split(separator)[tree.getFile().split(
												separator).length - 1] + ".prb").renameTo(new File(
							...
							if (tab.getTitleAt(i)
								...
								if (tree.getFile().length() > 4
									...
									tab.setTitleAt(i, rename);
										...
										&& (tree.getFile().substring(tree.getFile().length() - 4)
												.equals(".grf") || tree.getFile().substring(
												tree.getFile().length() - 4).equals(".prb"))) {
				...
				for (int i = 0; i < ((JTabbedPane) tab.getComponentAt(index)).getTabCount(); i++) {
					if (((JTabbedPane) tab.getComponentAt(index)).getComponentAt(i).getName()
						...
						if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Learn) {
							...
							.equals("Synthesis")) {
						if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Synthesis) {
							if (((Synthesis) ((JTabbedPane) tab.getComponentAt(index)).getComponent(i))
									.hasChanged()) {
								Object[] options = { "Yes", "No", "Cancel" };
								int value = JOptionPane.showOptionDialog(frame,
								...
								if (value == JOptionPane.YES_OPTION) {
									if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Synthesis) {
										((Synthesis) ((JTabbedPane) tab.getComponentAt(index))
												.getComponent(i)).save();
									}
								}
								else if (value == JOptionPane.CANCEL_OPTION) {
							...
							.equals("Verification")) {
						if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Verification) {
							if (((Verification) ((JTabbedPane) tab.getComponentAt(index)).getComponent(i))
									.hasChanged()) {
								Object[] options = { "Yes", "No", "Cancel" };
								int value = JOptionPane.showOptionDialog(frame,
								...
								if (value == JOptionPane.YES_OPTION) {
									if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Verification) {
										((Verification) ((JTabbedPane) tab.getComponentAt(index))
												.getComponent(i)).save();
									}
								}
								else if (value == JOptionPane.CANCEL_OPTION) {
				...
				if (tree.getFile().length() >= 5
					...
					try {
						...
						if (i != -1) {
							...
							addTab(theFile, new GCM2SBMLEditor(work.getAbsolutePath(), theFile,
									this, log), "GCM Editor");
						}
					}
					catch (Exception e1) {
	...
	private void openLearn() {
		...
		if (!done) {
			...
			lrnTab.addTab("TSD Graph", new Graph(null, "amount",
					tree.getFile().split(separator)[tree.getFile().split(separator).length - 1]
	...
	private void openSynth() {
		...
		if (!done) {
			...
			// if (!graphFile.equals("")) {
			synthPanel.add(new Synthesis(tree.getFile(), "flag", log, this));
			addTab(tree.getFile().split(separator)[tree.getFile().split(separator).length - 1],
					synthPanel, null);
		}
	}
	...
	private void openVerify() {
		...
		if (!done) {
			...
			// if (!graphFile.equals("")) {
			verPanel.add(new Verification(tree.getFile(), "flag", log, this));
			addTab(tree.getFile().split(separator)[tree.getFile().split(separator).length - 1],
					verPanel, null);
		}
	}
