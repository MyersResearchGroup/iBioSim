...
public class BioSim implements MouseListener, ActionListener, MouseMotionListener,
	...
	public void about() {
		...
		if (lema) {
			...
			version = new JLabel("Version 1.1.2", JLabel.CENTER);
	...
	public void actionPerformed(ActionEvent e) {
		...
		// if the Graph data menu item is clicked
			...
			if (tree.getFile().length() >= 4
				...
				lhpnFile.load(root + separator + tree.getFile());
				log.addText("Creating state graph.");
				StateGraph sg = new StateGraph(lhpnFile);
				sg.outputStateGraph(root + separator + (tree.getFile().replace(".lpn", ".dot")));
				String command = "";
				if (System.getProperty("os.name").contentEquals("Linux")) {
					// directory = ENVVAR + "/docs/";
					command = "gnome-open ";
				...
				log.addText("Executing:\n" + command + root + separator
						+ (tree.getFile().replace(".lpn", ".dot")) + "\n");
				Runtime exec = Runtime.getRuntime();
				File work = new File(root);
				try {
					exec.exec(command + (tree.getFile().replace(".lpn", ".dot")), null, work);
				}
				catch (IOException e1) {
	...
	public void updateAsyncViews(String updatedFile) {
		...
		for (int i = 0; i < tab.getTabCount(); i++) {
			...
			if (new File(properties).exists()) {
				...
				Verification verify = ((Verification) (((JPanel) this.tab.getComponentAt(i))
						.getComponent(0)));
