...
public class BioSim implements MouseListener, MouseMotionListener, ActionListener, FocusListener {
	...
	private JMenu file, edit, view, tools, help; // The file menu
	...
	public BioSim(boolean lema) {
		...
		view = new JMenu("View");
		JMenu viewModel = new JMenu("Model");
		...
		saveAsSbml = new JMenuItem("SBML Model");
		saveAsTemplate = new JMenuItem("SBML Template");
		...
		//saveAsMenu.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, ShortCutKey | KeyEvent.ALT_DOWN_MASK));
		//importMenu.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_I, ShortCutKey | KeyEvent.ALT_DOWN_MASK));
		//exportMenu.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, ShortCutKey | KeyEvent.ALT_DOWN_MASK));
		//viewModel.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_M, ShortCutKey | KeyEvent.ALT_DOWN_MASK));
		...
		//probGraph.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_H, ShortCutKey));
		//if (!lema) {
	...
	public void preferences() {
		if (!lema) {
			...
			Object[] options = { "Save", "Cancel" };
			// int value =
			JOptionPane.showOptionDialog(frame, prefTabsNoLema, "Preferences", JOptionPane.YES_NO_OPTION,
		...
		// if the run button is selected on the tool bar
			...
			if (comp instanceof JTabbedPane) {
				...
				if (component instanceof Graph) {
					...
					((Reb2Sac) (((JTabbedPane) comp).getComponent(index))).save();
					((Reb2Sac) (((JTabbedPane) comp).getComponent(index))).run();
				}
				else if (component instanceof Reb2Sac) {
					((Reb2Sac) component).save();
					((Reb2Sac) component).run();
				}
					...
					if (lhpnName != null && !lhpnName.trim().equals("")) {
						...
						if (!(IDpat.matcher(modelID).matches())) {
							...
							if (overwrite(root + separator + lhpnName, lhpnName)) {
								...
								addTab(f.getName(), new LHPNEditor(root + separator, f.getName(), null, this, log),
										"LHPN Editor");
								refreshTree();
							}
							if (overwrite(root + separator + lhpnName, lhpnName)) {
				...
				if (lrnName != null && !lrnName.trim().equals("")) {
					...
					try {
						if (overwrite(root + separator + lrnName, lrnName)) {
							...
							Graph tsdGraph = new Graph(null, "amount", lrnName + " data", "tsd.printer", root
									+ separator + lrnName, "time", this, null, log, null, true, true);
					...
					if (((JTabbedPane) tab.getComponentAt(index)).getComponentAt(i).getName().equals(
						...
						if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Synthesis) {
							if (((Synthesis) ((JTabbedPane) tab.getComponentAt(index)).getComponent(i))
								...
								if (value == JOptionPane.YES_OPTION) {
									if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Synthesis) {
										((Synthesis) ((JTabbedPane) tab.getComponentAt(index)).getComponent(i)).save();
									}
								}
								else if (value == JOptionPane.CANCEL_OPTION) {
									return 0;
						...
						if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Verification) {
							if (((Verification) ((JTabbedPane) tab.getComponentAt(index)).getComponent(i))
								...
								if (value == JOptionPane.YES_OPTION) {
									if (((JTabbedPane) tab.getComponentAt(index)).getComponent(i) instanceof Verification) {
										((Verification) ((JTabbedPane) tab.getComponentAt(index)).getComponent(i))
												.save();
									}
								}
								else if (value == JOptionPane.CANCEL_OPTION) {
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (e.getComponent() instanceof JMenu) {
			...
			if (e.getComponent() instanceof JMenu) {
				if (!((JMenu) e.getComponent()).isSelected()) {
					menuSelected = false;
				}
			}
		}
