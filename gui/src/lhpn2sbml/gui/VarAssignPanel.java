...
public class VarAssignPanel extends JPanel implements ActionListener {
	...
	public VarAssignPanel(String transition, String selected, PropertyList assignmentList, PropertyList continuousList,
		...
		varBox = new JComboBox(contList);
		varBox.setSelectedItem(contList[0]);
	...
	public void save() {
		...
		if (selected != null && !oldName.equals(id)) {
			...
			lhpn.getTransition(selectArray[0]).removeContAssign(oldArray[0]);
		}
		else {
			//System.out.println(transition + " " + id + " " + property.getProperty("Assignment value"));
			lhpn.getTransition(transition).addContAssign(id, property.getProperty("Value"));
		}
	}
