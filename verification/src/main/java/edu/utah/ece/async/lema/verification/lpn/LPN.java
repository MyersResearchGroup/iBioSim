...
import edu.utah.ece.async.biosim.dataModels.util.MutableString;
...
public class LPN extends Observable {
	...
	public static LPN convertToLHPN(ArrayList<String> specs, ArrayList<Object[]> conLevel, MutableString lpnProperty,
		...
		ArrayList<String> biochemical = bioModel.getBiochemicalSpecies();
		LPN LHPN = new LPN();
		for (int i = 0; i < specs.size(); i++) {
			if (sbml.getModel().getSpecies(specs.get(i))==null) continue;
			double initial = BioModel.parseValue(bioModel.getParameter(GlobalConstants.INITIAL_STRING));
		...
		for (String input : bioModel.getInputSpecies()) {
			double value;
			try {
		...
		for (String species : bioModel.getSpecies()) {
			if (!specs.contains(species) && !bioModel.getInputSpecies().contains(species)) {
				double value;
				try {
		...
		for (int i = 0; i < specs.size(); i++) {
			if (sbml.getModel().getSpecies(specs.get(i))==null) continue;
			if (!biochemical.contains(specs.get(i)) && !bioModel.getInputSpecies().contains(specs.get(i))) {
				int placeNum = 0;
				int transNum = 0;
				...
				for (Object threshold : conLevel.get(i)) {
					...
					if (!(bioModel.isInput(specs.get(i)))) {
						ArrayList<String> proms = new ArrayList<String>();
						Double global_np = BioModel.parseValue(bioModel.getParameter(GlobalConstants.STOICHIOMETRY_STRING));
						Double global_kd = BioModel.parseValue(bioModel.getParameter(GlobalConstants.KDECAY_STRING));
						Double np = global_np;
						Double kd = global_kd;
...
//							}
							...
							if (!BioModel.isProductionReaction(r) && !BioModel.isDegradationReaction(r)
								...
								for (int k = 0; k < r.getReactantCount(); k++) {
									if (r.getReactant(k).getSpecies().equals(specs.get(i))) {
										...
										for (String parameter : bioModel.getParameters()) {
											parameters.put(parameter, bioModel.getParameter(parameter));
										}
										...
										if (r.isSetReversible() && law.getMath().getCharacter() == '-') {
											reactionDegradations += "(("
													+ SBMLutilities.myFormulaToString(bioModel.replaceParams(law.getMath().getLeftChild(),
															parameters)) + ")*" + r.getReactant(k).getStoichiometry()
													+ ") + ";
											reactionProductions += "(("
													+ SBMLutilities.myFormulaToString(bioModel.replaceParams(law.getMath().getRightChild(),
															parameters)) + ")*" + r.getReactant(k).getStoichiometry()
													+ ") + ";
										}
										else {
											reactionDegradations += "(("
													+ SBMLutilities.myFormulaToString(bioModel.replaceParams(law.getMath(),
															parameters)) + ")*" + r.getReactant(k).getStoichiometry()
													+ ") + ";
										}
									}
						...
						kd = global_kd;
						Reaction reaction = bioModel.getDegradationReaction(specs.get(i));
						if (reaction != null) {
		...
		if (lpnProperty.getString() != null) {
			...
			for (String s : bioModel.getSpecies()) {
				sortedSpecies.add(s);
			}
			int i, j;
			String index;
	...
	public static void convertLPN2PRISM(FileWriter logFile,LPN LPN,String filename, SBMLDocument sbml) {
