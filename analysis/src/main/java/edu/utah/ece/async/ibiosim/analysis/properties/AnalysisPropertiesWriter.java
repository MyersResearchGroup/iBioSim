...
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.complex_formation_abstraction;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.gcm_Abstraction;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_min_print_interval;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_min_time_step;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_number_steps;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_out_dir;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_print_interval;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_runs;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_seed;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_start_index;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_time_limit;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.mc_time_step;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_abs_error;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_min_print_interval;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_min_time_step;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_number_steps;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_out_dir;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_print_interval;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_rel_error;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_time_limit;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.ode_time_step;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.operator_site_abstraction;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_abstraction;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_abstraction_1;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_abstraction_2;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_abstraction_3;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_concentration_level;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_diff_stoich_ampl;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_interesting_species;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_issa_adaptive;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_issa_paths;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_issa_type;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_max_operator_threshold;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_qssa_1;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_rapid_equil_1;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_rapid_equil_2;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_simulation;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_stats;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.reb2sac_transform;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.sim_init_time;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.sim_out_time;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.sim_printer;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.sim_run_term;
import static edu.utah.ece.async.ibiosim.analysis.properties.PropertiesConstants.sim_tracking_quantity;
...
public class AnalysisPropertiesWriter {
	 ...
	 *          - the object that is being exported to a properties file
	 * @throws IOException
	 *           - if there is a problem writing the properties file.
	 */
	public static void createProperties(AnalysisProperties analysisProperties) throws IOException {
		...
		if (analysisProperties.isOde()) {
			...
			if (analysisProperties.isPrintInterval()) {
				...
				properties.setProperty(ode_number_steps, String.valueOf((analysisProperties.getSimulationProperties().getNumSteps())));
			}
		...
		if (analysisProperties.isSsa()) {
			...
			if (analysisProperties.isPrintInterval()) {
				...
				properties.setProperty(mc_number_steps, "" + (analysisProperties.getSimulationProperties().getNumSteps()));
			}
			if (analysisProperties.getSimulationProperties().getMaxTimeStep() == Double.MAX_VALUE) {
	 ...
	 * 
	 * @param sedmlDoc
	 *          - a given SED-ML object that the given {@link AnalysisProperties} is being loaded to.
	 * @param properties
	 *          - object containing the property values.
	 */
