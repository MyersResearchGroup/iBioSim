...
public final class MathInterpreter
{
  ...
  {
    return parseASTNode(math, null, variableToNodes, dimensionNode, null);
  }

  public static HierarchicalNode parseASTNode(ASTNode math, Map<String, VariableNode> variableToNodes, VariableNode parent)
  {
    return parseASTNode(math, null, variableToNodes, null, parent);
  }

	public static HierarchicalNode parseASTNode(ASTNode math,  Map<String, HierarchicalNode> args, Map<String, VariableNode> variableToNodes, Map<String, VariableNode> dimensionNode)
	{
		return parseASTNode(math, args, variableToNodes, dimensionNode, null);
	}

	public static HierarchicalNode parseASTNode(ASTNode math, Map<String,HierarchicalNode> args, Map<String, VariableNode> variableToNodes, VariableNode parent)
	{
		return parseASTNode(math, args, variableToNodes, null, parent);
	}

	public static HierarchicalNode parseASTNode(ASTNode math, Map<String, VariableNode> variableToNodes)
	{
		return parseASTNode(math, null, variableToNodes, null, null);
	}

	public static HierarchicalNode parseASTNode(ASTNode math, Map<String, HierarchicalNode> args, Map<String, VariableNode> variableToNodes, Map<String, VariableNode> dimensionNodes, VariableNode parent)
	{
		...
		case CONSTRUCTOR_PIECE:
			...
			{
				node.addChild(parseASTNode(child, args, variableToNodes, dimensionNodes, parent));
			}
			break;
		...
		case FUNCTION:
	      ...
	      for (ASTNode child : math.getListOfNodes())
	      {
	        node.addChild(parseASTNode(child, args,variableToNodes, dimensionNodes));
	      }
		  }
		  else
		  {
       ...
       node = parseASTNode(func.getBody(), args, dimensionNodes, parent);
		  }
			
			break;
		case FUNCTION_ABS:
			node = new HierarchicalNode(Type.FUNCTION_ABS);
			node.addChild(parseASTNode(math.getChild(0), args, variableToNodes, parent));
			break;
...
//			  if(node.isSpecies())
//			  {
//	        ((SpeciesNode) node).addReactionDependency((ReactionNode) parent);
//			  }
//			}
			break;
		case NAME_AVOGADRO:
