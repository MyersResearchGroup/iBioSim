...
public final class MathInterpreter {
  ...
  public static HierarchicalNode parseASTNode(ASTNode math, Map<String, HierarchicalNode> args, Map<String, HierarchicalNode> variableToNodes, Map<String, HierarchicalNode> dimensionNodes, int index) {
    ...
    case FUNCTION:
      if (HierarchicalUtilities.ibiosimFunctionDefinitions.contains(math.getName())) {
        ...
        for (int i = 0; i < func.getNumArguments(); i++) {
          ...
          HierarchicalNode parseArg = parseASTNode(child, args, variableToNodes, dimensionNodes, index);

          functArgs.put(arg.getName(), parseArg);
        }
        node = parseASTNode(func.getBody(), functArgs, variableToNodes, dimensionNodes, index);
      }
    ...
    case FUNCTION_ARCCSCH:
      ...
      node.addChild(parseASTNode(math.getChild(0), args, variableToNodes, null, index));
      break;
