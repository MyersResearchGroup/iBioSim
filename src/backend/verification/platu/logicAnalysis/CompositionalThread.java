...
//		int[] otherIndexList = sg.getOtherIndexArray(index);
	...
	private static boolean compatible(backend.verification.platu.stategraph.State currentState, Constraint constr, int[] thisIndexList, int[] otherIndexList){
     ...
     */
	@SuppressWarnings("unused")
	private static int createNewState(StateGraph sg, backend.verification.platu.stategraph.State compatibleState, Constraint c){
		int newTransitions = 0;

		// Create new state and insert into state graph
		backend.verification.platu.stategraph.State newState = new backend.verification.platu.stategraph.State(compatibleState);
		...
		backend.verification.platu.stategraph.State nextState = sg.addState(newState);
