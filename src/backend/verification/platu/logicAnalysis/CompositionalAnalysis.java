	...
	public static CompositeStateGraph compose2(CompositeStateGraph sg1, CompositeStateGraph sg2){
		...
		while(!newStateStack.isEmpty()){
			...
			for(CompositeStateTran stateTran : stateTrans1){
				main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran = stateTran.getLPNTran();
				...
				if(!stateTran.visible()){
					...
					if(synchronousTrans.contains(lpnTran.getIndex())){
						for(CompositeStateTran stateTran2 : stateTrans2){
							main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran2  = stateTran2.getLPNTran();
							if(lpnTran == lpnTran2){
...
//					System.out.println(newStateTran);
			...
			while(iter1.hasNext()){
				...
				main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran = stateTran1.getLPNTran();
...
//					System.out.println(newStateTran);
	...
	public static CompositeStateGraph compose(CompositeStateGraph sg1, CompositeStateGraph sg2){
		...
		List<main.java.edu.utah.ece.async.lpn.parser.Transition> intersectingTrans = new ArrayList<main.java.edu.utah.ece.async.lpn.parser.Transition>();
		...
		List<main.java.edu.utah.ece.async.lpn.parser.Transition> independentTrans1 = new ArrayList<main.java.edu.utah.ece.async.lpn.parser.Transition>();
		List<CompositeState> indStateList1 = new ArrayList<CompositeState>();
		List<main.java.edu.utah.ece.async.lpn.parser.Transition> independentTrans2 = new ArrayList<main.java.edu.utah.ece.async.lpn.parser.Transition>();
		...
		while(!newStateStack.isEmpty()){
			...
			for(int i = 0; i < independentTrans1.size(); i++){
				main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran = independentTrans1.get(i);
...
//					System.out.println(newStateTran);
			...
			for(int j = 0; j < independentTrans2.size(); j++){
				main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran = independentTrans2.get(j);
...
//			if(!sg.containsStateTran(nonlocalStateTran)) continue;
			...
			main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran = nonlocalStateTran.getLPNTran();
...
//			// if state has already been encountered skip
			...
			for(CompositeStateTran succStateTran : nextState.getOutgoingStateTranList()){
				main.java.edu.utah.ece.async.lpn.parser.Transition lpnTran = succStateTran.getLPNTran();
