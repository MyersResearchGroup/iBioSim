...
public class DBM {
	...
	public void canonicalize() {
		...
		for(int x = 0; x < dim; x++)
			for(int y = 0; y < dim; y++) {
				...
				for(int i = 0; i < dim; i++) {
					if(i==x || i==y) continue;
					if(matrix[x][i]==backend.verification.platu.common.Common.INFINITY || matrix[i][y]==backend.verification.platu.common.Common.INFINITY)
						continue;
	...
	public String toString() {
    	...
    	for(int x = 0; x < dim; x++) {
        	for(int y = 0; y < dim; y++) {
        		if(this.matrix[x][y] == backend.verification.platu.common.Common.INFINITY)
        			strOut  += "\tINF";
        		else
